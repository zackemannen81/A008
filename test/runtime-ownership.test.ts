...
function processOwner() {
  const child = fork(
    new URL("./fixtures/runtime-owner-process.js", import.meta.url),
    [],
    { silent: true },
  );
...
test("process ownership rejects canonical aliases before creation and releases on close or crash", async () => {
  ...
  try {
    for (const crash of [false, true]) {
      const child = processOwner();
      let created = 0;
      const contender = new ProjectRuntimeRegistry({
        env: f.env,
      ...
      try {
        const first = await child.open(f.env, cwd);
...
test("process namespace leases allow separate projects in one SQLite file and preserve seeded data", async () => {
  const f = isolatedMemoryEnv(),
    a = join(f.directory, "a"),
    b = join(f.directory, "b");
  mkdirSync(a);
  mkdirSync(b);
  ...
  for (const id of [TEST_PROJECT_ID, second]) {
    ...
    seed.runtime.writeSharedMemory({ content: `Seeded namespace ${id}` });
    seed.runtime.close();
  }
  const child = processOwner(),
    registry = new ProjectRuntimeRegistry({ env: f.env });
  try {
    const first = await child.open(f.env, a, {
    ...
    const other = registry.attachExisting({
      cwd: b,
      projectId: second,
      sqlitePath: f.sqlitePath,
    });
    assert.ok(
      other.runtime
        .inspectMemory()
        .records.some((row) => row.detail.includes(second)),
    );
    assert.ok(
      !other.runtime
        .inspectMemory()
        .records.some((row) => row.detail.includes(TEST_PROJECT_ID)),
    );
  } finally {
    await child.close(true);
    registry.close();
  ...
  try {
    ...
    try {
      assert.equal(
        ...
        results.find((result) => result.ok)!.projectId,
      );
    } finally {
      registry.close();
    }
  ...
  try {
    for (const surface of ["cli", "acp"] as const) {
      ...
      try {
        ...
        assert.match(denied.error!, /process owner/u);
        registry.close();
        const allowed = await child.open(f.env, cwd, undefined, surface);
        assert.equal(allowed.ok, true, allowed.error);
        const contender = new ProjectRuntimeRegistry({ env: f.env });
        try {
          assert.throws(
            () => contender.openConfigured(cwd, f.env),
            /process owner/u,
          );
        } finally {
          contender.close();
