...
test("engine panel shares external session, settings, stream and project memory; closing panel keeps session", async () => {
  ...
  const workspace = join(f.directory, "project");
  mkdirSync(workspace);
  ...
  try {
    ...
    wire.send({
      ...
      control: {
        ...
        settings: {
          ...old.settings,
          instructions: "Engine instruction fixture.",
        },
      },
    });
    assert.equal(
      (await wire.until("session/control/ok", "settings")).state
        .runtimePreferences.settings.instructions,
      "Engine instruction fixture.",
    );
    const notifications: SessionNotification[] = [];
    await engine.prompt(
      {
        sessionId: created.sessionId,
        prompt: [{ type: "text", text: "Native client question." }],
      },
      async (n) => {
    ...
    const configured = provider.requests.find((r) =>
      r.messages.some(
        (m: any) =>
          m.role === "system" &&
          m.content.includes("Engine instruction fixture."),
      ),
    );
    assert.ok(configured);
    assert.deepEqual(
      configured.messages.filter((m: any) => m.role === "system"),
      [
        {
          role: "system",
          content: `Engine instruction fixture.\n\n${MEMORY_CONTEXT_SYSTEM_INSTRUCTION}`,
        },
      ],
    );
    const upload = await fetch(`${origin}/v1/upload`, {
      ...
      headers: {
        ...
        "x-a008-filename": "engine.txt",
      },
      body: "Engine upload fixture.",
    });
    assert.equal(upload.status, 200);
    const memory = (await (
      await fetch(`${origin}/v1/memory`, { headers })
    ).json()) as any;
    ...
    assert.equal(
      engine.control(created.sessionId, { action: "inspect" }).messages.length,
      4,
    );
    assert.ok(
      readFileSync(f.env.A008_SETTINGS_PATH!, "utf8").includes(
        "Engine instruction fixture.",
      ),
    );
  } finally {
...
test("engine project selection isolates runtime identities and memory and rejects ambiguous memory access", async () => {
  ...
  try {
    ...
    assert.equal(
      engine.control(a2.sessionId, { action: "inspect" }).runtime.projectId,
      sa.runtime.projectId,
    );
    assert.throws(() => engine.projectAgent({}), /Select a project/);
    await assert.rejects(
      engine.newSession({ cwd: ".", mcpServers: [] }),
      /absolute/,
    );
    await engine.closeSession(a.sessionId);
    assert.throws(() => engine.sessionAgent(a.sessionId), /Unknown/);
    assert.equal(
      engine.control(a2.sessionId, { action: "inspect" }).messages.length,
      0,
    );
  } finally {
    await engine.close();
...
test("explicit legacy attachment preserves its store and reports model and close changes to the client", async () => {
  const f = isolatedMemoryEnv();
  const workspace = join(f.directory, "legacy");
  mkdirSync(workspace);
  ...
  try {
    assert.equal(
      engine.sharedMemoryCapabilities({
    ...
    assert.equal(
      engine.control(created.sessionId, { action: "inspect" }).runtime
        .memoryPath,
      f.env.A008_MEMORY_SQLITE_PATH,
    );
    assert.equal(
      engine.projectAgent({ projectId: `local:${workspace}` }),
      engine.sessionAgent(created.sessionId),
    );
    engine.control(created.sessionId, {
      action: "model",
    ...
    assert.ok(
      notifications.some((n) => n.update._meta?.["engine.closed"] === true),
    );
  } finally {
    await engine.close();
    rmSync(f.directory, { recursive: true, force: true });
