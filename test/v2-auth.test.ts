...
import { v2ErrorSchema, v2InfoSchema, v2SessionAuthenticatedSchema, v2SessionResultSchema, v2SessionServerFrameSchema, v2TicketResponseSchema } from "../packages/protocol/src/index.js";
...
class V2WireClient {
  ...
  constructor(readonly socket: WebSocket) {
    socket.addEventListener("message", event => {
      ...
      assert.ok(v2SessionServerFrameSchema.safeParse(frame).success, `Invalid V2 server frame: ${JSON.stringify(frame)}`);
  ...
  static async connect(port: number, ticket?: string): Promise<V2WireClient> {
    const socket = new WebSocket(`ws://127.0.0.1:${port}/v2/session`, "a008.v2");
    const client = new V2WireClient(socket);
    await new Promise<void>((resolve, reject) => {
      socket.addEventListener("open", () => resolve(), { once: true });
      socket.addEventListener("error", () => reject(new Error("V2 WebSocket failed to open.")), { once: true });
    });
    if (ticket) {
      client.send({ type: "authenticate", ticket });
      const authenticated = await client.next();
      assert.ok(v2SessionAuthenticatedSchema.safeParse(authenticated).success, JSON.stringify(authenticated));
    }
    return client;
  }
  ...
  next(timeoutMs = 4000): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => reject(new Error("Timed out waiting for V2 frame.")), timeoutMs);
      this.#waiters.push(frame => { clearTimeout(timer); resolve(frame); });
    });
  }

  async until(type: string, requestId?: string, timeoutMs = 8000): Promise<any> {
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
      const frame = await this.next(Math.max(1, deadline - Date.now()));
      if (frame.type === type && (requestId === undefined || frame.requestId === requestId)) return frame;
    }
...
async function issueV2Ticket(port: number, credential: string, projectId: string, sessionId?: string) {
  ...
  assert.ok(v2TicketResponseSchema.safeParse(body).success, JSON.stringify(body));
  return body.ticket as string;
}

function createRegisteredFixtureProject(f: ReturnType<typeof fixture>, name = "V2 fixture") {
  return executeProjectBootstrap(parseProjectBootstrapConfig({
    projectName: name,
  ...
  }), { registryPath: f.env.A008_PROJECTS_PATH! }).project;
}
test("real V2 WebSocket authenticates once and runs project-bound session operations", { timeout: 30000 }, async () => {
  ...
  const devices = new DeviceRegistry(f.env);
  const grant = devices.grant({ name: "V2 client", projects: [project.projectId], capabilities: ["session"] });
  const host = await startGuiHost({ env: f.env, port: 0, cwd: f.directory });
  let client: V2WireClient | undefined;
  try {
    const ticket = await issueV2Ticket(host.port, grant.credential, project.projectId);
    client = await V2WireClient.connect(host.port, ticket);
    client.send({ type: "command", requestId: "new_1", action: "session/new", projectId: project.projectId });
    const opened = await client.until("result", "new_1");
    assert.ok(v2SessionResultSchema.safeParse(opened).success, JSON.stringify(opened));
    ...
    client.send({ type: "command", requestId: "prompt_1", action: "session/prompt", projectId: project.projectId,
      sessionId, payload: { text: "Hello through V2" } });
    ...
    assert.ok(client.frames.some(frame => frame.type === "signal" && frame.signal === "answer" && frame.sessionId === sessionId));

    client.send({ type: "command", requestId: "inspect_1", action: "session/inspect", projectId: project.projectId, sessionId });
...
function waitForSocketClose(socket: WebSocket, timeoutMs = 7000): Promise<number> {
  if (socket.readyState === WebSocket.CLOSED) return Promise.resolve(1000);
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error("Timed out waiting for V2 socket close.")), timeoutMs);
    socket.addEventListener("close", event => { clearTimeout(timer); resolve(event.code); }, { once: true });
  });
}

async function expectV2Error(client: V2WireClient, codeValue: string, requestId?: string) {
  while (true) {
...
test("V2 WebSocket enforces subprotocol, pre-auth deadline, frame bounds and ticket one-use", { timeout: 20000 }, async () => {
  const f = fixture();
  const project = createRegisteredFixtureProject(f, "V2 limits");
  const devices = new DeviceRegistry(f.env);
  const grant = devices.grant({ name: "Limits client", projects: [project.projectId], capabilities: ["session"] });
  const host = await startGuiHost({ env: f.env, port: 0, cwd: f.directory });
  const clients: V2WireClient[] = [];
  try {
    const ticket = await issueV2Ticket(host.port, grant.credential, project.projectId);
    const first = await V2WireClient.connect(host.port, ticket); clients.push(first);

    const reused = await V2WireClient.connect(host.port); clients.push(reused);
    reused.send({ type: "authenticate", ticket });
    await expectV2Error(reused, "UNAUTHENTICATED");
    await waitForSocketClose(reused.socket);

    const oversizedPreauth = await V2WireClient.connect(host.port); clients.push(oversizedPreauth);
    oversizedPreauth.socket.send("x".repeat(4097));
    await waitForSocketClose(oversizedPreauth.socket);

    const late = await V2WireClient.connect(host.port); clients.push(late);
    ...
    const secondTicket = await issueV2Ticket(host.port, grant.credential, project.projectId);
    const oversizedAuthed = await V2WireClient.connect(host.port, secondTicket); clients.push(oversizedAuthed);
...
test("V2 session authority rejects foreign project, principal and second writer", { timeout: 20000 }, async () => {
  const f = fixture();
  const projectA = createRegisteredFixtureProject(f, "Project A");
  const projectB = createRegisteredFixtureProject(f, "Project B");
  const devices = new DeviceRegistry(f.env);
  const owner = devices.grant({ name: "Owner A", projects: [projectA.projectId, projectB.projectId], capabilities: ["session"] });
  const foreign = devices.grant({ name: "Foreign A", projects: [projectA.projectId], capabilities: ["session"] });
  const host = await startGuiHost({ env: f.env, port: 0, cwd: f.directory });
  const clients: V2WireClient[] = [];
  try {
    const ownerClient = await V2WireClient.connect(host.port, await issueV2Ticket(host.port, owner.credential, projectA.projectId));
    clients.push(ownerClient);
    ownerClient.send({ type: "command", requestId: "new_auth", action: "session/new", projectId: projectA.projectId });
    const opened = await ownerClient.until("result", "new_auth");
    const sessionId = opened.sessionId as string;

    ownerClient.send({ type: "command", requestId: "wrong_project", action: "session/inspect", projectId: projectB.projectId, sessionId });
    await expectV2Error(ownerClient, "FORBIDDEN", "wrong_project");

    const foreignClient = await V2WireClient.connect(host.port, await issueV2Ticket(host.port, foreign.credential, projectA.projectId));
    clients.push(foreignClient);
    foreignClient.send({ type: "command", requestId: "foreign_session", action: "session/inspect", projectId: projectA.projectId, sessionId });
    await expectV2Error(foreignClient, "SESSION_EXPIRED", "foreign_session");

    const scopedTicket = await issueV2Ticket(host.port, owner.credential, projectA.projectId, sessionId);
    const secondWriter = await V2WireClient.connect(host.port, scopedTicket); clients.push(secondWriter);
    secondWriter.send({ type: "command", requestId: "second_writer", action: "session/inspect", projectId: projectA.projectId, sessionId });
...
async function nextV2Signal(client: V2WireClient, signal: string, timeoutMs = 8000): Promise<any> {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
...
test("V2 tool approval executes once and live revoke denies pending work", { timeout: 30000 }, async () => {
  const provider = await startSessionControlProvider(payload => {
    const messages = payload.messages ?? [];
    const last = messages.at(-1);
    if (last?.role === "tool") return { role: "assistant", content: "Tool phase finished." };
    const user = [...messages].reverse().find((message: any) => message.role === "user")?.content ?? "";
    if (!String(user).includes("WRITE")) return undefined;
    const filename = String(user).includes("REVOKE") ? "revoked.txt" : "approved.txt";
    return { role: "assistant", content: null, tool_calls: [{ id: `write-${filename}`, type: "function",
      function: { name: "create_file", arguments: JSON.stringify({ path: filename, content: filename }) } }] };
  });
  const f = fixture();
  f.env.NVIDIA_CHAT_COMPLETIONS_URL = provider.endpoint; f.env.PATH = process.env.PATH;
  const project = createRegisteredFixtureProject(f, "V2 tools");
  const devices = new DeviceRegistry(f.env);
  const grant = devices.grant({ name: "Tool client", projects: [project.projectId], capabilities: ["session"] });
  const host = await startGuiHost({ env: f.env, port: 0, cwd: f.directory });
  let client: V2WireClient | undefined;
  try {
    client = await V2WireClient.connect(host.port, await issueV2Ticket(host.port, grant.credential, project.projectId));
    client.send({ type: "command", requestId: "new_tools", action: "session/new", projectId: project.projectId });
    const sessionId = (await client.until("result", "new_tools")).sessionId as string;

    client.send({ type: "command", requestId: "prompt_allow", action: "session/prompt", projectId: project.projectId,
      sessionId, payload: { text: "WRITE APPROVE" } });
    ...
    client.send({ type: "command", requestId: "allow_1", action: "tool/permission", projectId: project.projectId,
      sessionId, payload: { permissionId: permission.permissionId, allow: true } });
    await client.until("result", "allow_1");
    await client.until("result", "prompt_allow");
    assert.equal(readFileSync(join(project.rootFolder, "approved.txt"), "utf8"), "approved.txt");

    client.send({ type: "command", requestId: "prompt_revoke", action: "session/prompt", projectId: project.projectId,
      sessionId, payload: { text: "WRITE REVOKE" } });
    ...
    client?.close(); await host.close(); await provider.close(); rmSync(f.directory, { recursive: true, force: true });
  }
});
test("V2 cancel and session/control dispatch through the shared session owner", { timeout: 20000 }, async () => {
  const provider = await startSessionControlProvider();
  const f = fixture();
  f.env.NVIDIA_CHAT_COMPLETIONS_URL = provider.endpoint; f.env.PATH = process.env.PATH;
  const project = createRegisteredFixtureProject(f, "V2 controls");
  const devices = new DeviceRegistry(f.env);
  const grant = devices.grant({ name: "Control client", projects: [project.projectId], capabilities: ["session"] });
  const host = await startGuiHost({ env: f.env, port: 0, cwd: f.directory });
  let client: V2WireClient | undefined;
  try {
    client = await V2WireClient.connect(host.port, await issueV2Ticket(host.port, grant.credential, project.projectId));
    client.send({ type: "command", requestId: "new_controls", action: "session/new", projectId: project.projectId });
    const sessionId = (await client.until("result", "new_controls")).sessionId as string;

    client.send({ type: "command", requestId: "control_inspect", action: "session/control", projectId: project.projectId,
      sessionId, payload: { control: { action: "inspect" } } });
    const controlled = await client.until("result", "control_inspect");
    assert.equal(controlled.state.sessionId, sessionId);

    client.send({ type: "command", requestId: "wait_prompt", action: "session/prompt", projectId: project.projectId,
      sessionId, payload: { text: "WAIT-TURN" } });
    await nextV2Signal(client, "thought");
    client.send({ type: "command", requestId: "cancel_wait", action: "session/cancel", projectId: project.projectId, sessionId });
    const cancelled = await client.until("result", "cancel_wait");
    assert.equal(cancelled.sessionId, sessionId);
    const promptResult = client.frames.find(frame => frame.type === "result" && frame.requestId === "wait_prompt") ??
...
test("V2 connection binding is transactional and fences concurrent session creation", { timeout: 15000 }, async () => {
  const f = fixture();
  const project = createRegisteredFixtureProject(f, "V2 binding race");
  const devices = new DeviceRegistry(f.env);
  const grant = devices.grant({ name: "Binding client", projects: [project.projectId], capabilities: ["session"] });
  const host = await startGuiHost({ env: f.env, port: 0, cwd: f.directory });
  let client: V2WireClient | undefined;
  try {
    client = await V2WireClient.connect(host.port, await issueV2Ticket(host.port, grant.credential, project.projectId));
    client.send({ type: "command", requestId: "missing_first", action: "session/inspect", projectId: project.projectId, sessionId: "missing_session" });
    await expectV2Error(client, "SESSION_EXPIRED", "missing_first");

    client.send({ type: "command", requestId: "race_a", action: "session/new", projectId: project.projectId });
    client.send({ type: "command", requestId: "race_b", action: "session/new", projectId: project.projectId });
    const seen: any[] = [];
    while (!seen.some(frame => frame.requestId === "race_a") || !seen.some(frame => frame.requestId === "race_b")) seen.push(await client.next());
    const raceFrames = seen.filter(frame => frame.requestId === "race_a" || frame.requestId === "race_b");
    assert.equal(raceFrames.filter(frame => frame.type === "result").length, 1, JSON.stringify(raceFrames));
    assert.equal(raceFrames.filter(frame => frame.type === "error" && frame.code === "SESSION_BUSY").length, 1, JSON.stringify(raceFrames));
  } finally {
    client?.close(); await host.close(); rmSync(f.directory, { recursive: true, force: true });
  }
});
test("V2 WebSocket rejects the wrong subprotocol on the real host", { timeout: 10000 }, async () => {
  const f = fixture();
  createRegisteredFixtureProject(f, "V2 protocol");
  const host = await startGuiHost({ env: f.env, port: 0, cwd: f.directory });
  try {
    await assert.rejects(new Promise<void>((resolve, reject) => {
      const socket = new WebSocket(`ws://127.0.0.1:${host.port}/v2/session`, "wrong.v2");
      socket.addEventListener("open", () => { socket.close(); resolve(); }, { once: true });
      socket.addEventListener("error", () => reject(new Error("wrong subprotocol refused")), { once: true });
    }), /wrong subprotocol refused/u);
  } finally {
    await host.close(); rmSync(f.directory, { recursive: true, force: true });
  }
});

test("live V2 device expiry closes an authenticated socket", { timeout: 10000 }, async () => {
  const f = fixture();
  const project = createRegisteredFixtureProject(f, "V2 expiry");
  const devices = new DeviceRegistry(f.env);
  const grant = devices.grant({ name: "Expiry client", projects: [project.projectId], capabilities: ["session"] });
  const host = await startGuiHost({ env: f.env, port: 0, cwd: f.directory });
  let client: V2WireClient | undefined;
  try {
    client = await V2WireClient.connect(host.port, await issueV2Ticket(host.port, grant.credential, project.projectId));
    const db = new Database(devices.path);
    try { db.prepare("UPDATE devices SET expires = ? WHERE id = ?").run(Date.now() - 1, grant.device.id); }
    finally { db.close(); }
    await waitForSocketClose(client.socket, 4000);
  } finally {
    client?.close(); await host.close(); rmSync(f.directory, { recursive: true, force: true });
...
test("V2 WebSocket output applies the existing credential redaction boundary", { timeout: 15000 }, async () => {
  const secret = "synthetic-v2-wire-secret";
  const provider = await startSessionControlProvider(() => ({ role: "assistant", content: `Never expose ${secret}.` }));
  const f = fixture();
  f.env.NVIDIA_API_KEY = secret; f.env.NVIDIA_CHAT_COMPLETIONS_URL = provider.endpoint; f.env.PATH = process.env.PATH;
  const project = createRegisteredFixtureProject(f, "V2 redaction");
  const devices = new DeviceRegistry(f.env);
  const grant = devices.grant({ name: "Redaction client", projects: [project.projectId], capabilities: ["session"] });
  const host = await startGuiHost({ env: f.env, port: 0, cwd: f.directory });
  let client: V2WireClient | undefined;
  try {
    client = await V2WireClient.connect(host.port, await issueV2Ticket(host.port, grant.credential, project.projectId));
    client.send({ type: "command", requestId: "new_redact", action: "session/new", projectId: project.projectId });
    const sessionId = (await client.until("result", "new_redact")).sessionId as string;
    client.send({ type: "command", requestId: "prompt_redact", action: "session/prompt", projectId: project.projectId,
      sessionId, payload: { text: "redact fixture" } });
