...
import { DEFAULT_RUNTIME_BUDGETS, parseRuntimePreferences, RUNTIME_BUDGET_FIELDS, type RuntimePreferences } from "../src/core/runtime-preferences.js";
...
import { DEFAULT_MODEL_ID, defaultModelRegistry } from "../src/core/model-registry.js";
...
function update(store: RuntimePreferencesStore, budgets: Partial<RuntimePreferences["budgets"]>, instructions?: string) {
  const before = store.snapshot();
  return store.save({ instructions: instructions ?? before.settings.instructions,
    budgets: { ...before.settings.budgets, ...budgets } }, before.revision);
}
...
test("all editable limits validate; disk saves are atomic, global and revision guarded", () => {
  ...
  try {
    ...
    const saved = update(a, { chatInputBytes: 19000, recentMessages: 0 }, identity);
    assert.deepEqual(b.snapshot(), saved);
    for (const f of RUNTIME_BUDGET_FIELDS) {
      for (const value of [f.minimum - 1, NaN, Infinity, 1.5, "100", f.maximum + 1]) {
        assert.throws(() => a.save({ ...saved.settings, budgets: { ...saved.settings.budgets, [f.key]: value } }, saved.revision), /whole number/);
      }
    }
    assert.throws(() => parseRuntimePreferences({ ...defaults(), extra: true }), /Invalid runtime/);
    assert.throws(() => b.save(defaults(), original.revision), /changed elsewhere/);
    ...
    assert.throws(() => new RuntimePreferencesStore({ A008_SETTINGS_PATH: join(process.cwd(), "settings.json") }, 180000), /outside/);
...
test("input-budget repair retains history; global instructions survive zero history, reset, model and project restart", async () => {
  ...
  let runtime = createLocalMemoryRuntime({ env: isolated.env, surface: "cli", createTransport: () => fake.transport });
  try {
    const session = runtime.openSession({ systemMessage: "Synthetic application system." });
    ...
    await assert.rejects(session.send(long), /Chat input.*hard budget.*16384.*utf8-bytes/);
    ...
    assert.equal(session.messages.some(m => m.content === identity), false);
    session.reset();
    ...
    runtime.close();
    runtime = createLocalMemoryRuntime({ env: { ...isolated.env, A008_MEMORY_SQLITE_PATH: ":memory:", A008_PROJECT_ID: "A008_v1_project_40000000-0000-4000-8000-000000000099" }, surface: "acp", createTransport: () => fake.transport });
...
test("memory output, input, staging and proposal limits reach the existing pipeline", async () => {
  ...
  try {
    ...
    update(runtime.preferences, { maximumProposals: 1, semanticOutputTokens: 512 });
    assert.equal((await session.turn("fixture facts")).postOutput.status, "staging_failed");
    assert.equal(fake.requests.filter(r => operation(r)).every(r => r.options?.maxTokens === 512), true);
    update(runtime.preferences, { maximumProposals: 2, stagingBytes: 1 });
    assert.equal((await session.turn("fixture facts")).postOutput.status, "staging_failed");
    update(runtime.preferences, { stagingBytes: 262144, semanticInputBytes: 1 });
    ...
    assert.equal(fake.requests.filter(r => operation(r)).length, semanticBefore);
    assert.match(rejected.memoryDiagnostic!, /Semantic input.*hard budget/);
    update(runtime.preferences, { semanticInputBytes: 262144, maximumTagsPerProposal: 1 });
    ...
    assert.equal("records" in succeeded.postOutput && succeeded.postOutput.records.length, 2);
  } finally { runtime.close(); rmSync(isolated.directory, { recursive: true, force: true }); }
});
...
test("an active turn keeps its instructions, semantic budgets and timeout when another process saves", async () => {
  ...
  update(external, { semanticOutputTokens: 1024, providerTimeoutMs: 4000 }, identity);
  const requests: { request: ChatRequest; timeout: number }[] = [];
  let changed = false;
  const runtime = createLocalMemoryRuntime({ env: isolated.env, surface: "cli", createTransport: options => ({ async complete(request) {
    requests.push({ request, timeout: options.timeoutMs! });
    if (!changed) { changed = true; update(external, { semanticOutputTokens: 2048, providerTimeoutMs: 9000 }, "A different saved instruction."); }
    return { message: { role: "assistant", content: operation(request) === "knowledge_analysis" ? "[]" : operation(request) ? '{"domains":[]}' : "Synthetic answer." } };
  } }) });
  try {
    const session = runtime.openSession();
...
test("OpenAI-only Luna runtime keeps chat and semantic scope off NVIDIA", async () => {
  const isolated = isolatedMemoryEnv({ NVIDIA_API_KEY: undefined, OPENAI_API_KEY: "sk-openai-fixture" });
  ...
  globalThis.fetch = async (input, init) => {
    ...
    calls.push({ url, body });
    let op: string | undefined;
    try { op = JSON.parse(body.messages.at(-1).content).operation; } catch { /* chat */ }
    if (body.stream === true) {
      return new Response(
    ...
    const content = op === "retrieval_scope"
  ...
  try {
    ...
    assert.deepEqual(calls.map(call => operation({ messages: call.body.messages } as ChatRequest) ?? "chat"),
      ["retrieval_scope", "chat", "knowledge_analysis"]);
    assert.equal(calls.every(call => call.url === "https://api.openai.com/v1/chat/completions"), true);
    assert.equal(calls.every(call => call.body.model === "gpt-5.6-luna"), true);
    assert.equal(calls.filter(call => call.body.stream === false).every(call => call.body.reasoning_effort === "none"), true);
    assert.equal(calls.find(call => call.body.stream === true)?.body.reasoning_effort, "medium");
  } finally {
    runtime.close();
...
test("cancelling the actual retrieval call stops before chat or memory writes", async () => {
  ...
  const requests: ChatRequest[] = [];
  const runtime = createLocalMemoryRuntime({ env: isolated.env, surface: "cli", createTransport: () => ({ complete(request) {
    requests.push(request);
    assert.equal(operation(request), "retrieval_scope");
    return new Promise((_resolve, reject) => {
      request.signal?.addEventListener("abort", () => reject(new ChatError("cancelled", "Cancelled fixture scope.")), { once: true });
      started();
    });
  } }) });
  try {
    ...
    await entered;
    assert.throws(() => session.configureRuntimePreferences(defaults(), session.runtimePreferences.revision), /active turn/);
    abort.abort();
...
test("scope and vocabulary budgets govern real runtime retrieval across turns", async () => {
  const isolated = isolatedMemoryEnv();
  const seed = createSqliteKnowledgeContext({ filename: isolated.sqlitePath, projectId: parseRuntimeId(isolated.env.A008_PROJECT_ID!, "project") });
  for (const domain of ["neuroscience", "sleep science"]) {
    const stored = ingest({ content: `${domain} fixture evidence.`, speaker: "user", locator: `fixture:${domain}`, scope: { verified: true, tags: ["local"] } }, { store: seed.context.evidence });
    seed.context.labels.attach({ recordId: stored.utterances[0]!.id, recordKind: "utterance", tags: ["fixture"], domains: [domain] });
  }
  seed.close();
  let scopeTurn = 0;
  ...
  const runtime = createLocalMemoryRuntime({ env: isolated.env, surface: "cli", createTransport: () => ({ async complete(request) {
    ...
    if (operation(request) === "retrieval_scope") {
      ...
      content = JSON.stringify({ domains: ++scopeTurn % 2 === 1 ? ["neuroscience", "sleep science"] : ["sleep science"], relatedDomains: [] });
    } else if (operation(request)) content = "[]";
    return { message: { role: "assistant", content } };
  } }) });
  try {
    update(runtime.preferences, { maximumScopeDomains: 1, retrievalVocabulary: 1 });
    const session = runtime.openSession();
    const initial = await session.turn("A deliberately unrelated first question");
    assert.ok(initial.memory.projection.projection.items.some(p => p.proposition.includes("neuroscience")));
    const narrowed = await session.turn("A deliberately unrelated follow-up");
    assert.equal(narrowed.memory.projection.projection.items.some(p => p.proposition.includes("neuroscience")), false);
    assert.equal(vocabularies[0].knownDomains.length, 1);
    update(runtime.preferences, { maximumScopeDomains: 2, retrievalVocabulary: 2 });
    await session.turn("Another unrelated question");
    const retained = await session.turn("Another unrelated follow-up");
    assert.ok(retained.memory.projection.projection.items.some(p => p.proposition.includes("neuroscience")));
...
test("retrieval and projection budgets take effect in an existing session without deleting evidence", async () => {
  ...
  const runtime = createLocalMemoryRuntime({ env: isolated.env, surface: "cli", createTransport: () => fake.transport });
  try {
    for (const word of ["first", "second", "third"]) runtime.writeSharedMemory({ content: `FixtureKnowledge ${word} ${"synthetic ".repeat(20)}` });
    const before = runtime.inspectMemory().summary.total;
    const session = runtime.openSession();
    update(runtime.preferences, { memoryProjectionBytes: 1, retrievalTerms: 1, retrievalEntities: 1 });
    const narrow = await session.turn('"FixtureKnowledge" "SecondEntity" "ThirdEntity" one two three');
    ...
    update(runtime.preferences, { memoryProjectionBytes: 65536, retrievalTerms: 10, retrievalEntities: 3, recentMessages: 4,
      retrievalHistoryMessages: 0, retrievalHistoryCharacters: 1, retrievalSemanticQueries: 1 });
    const wider = await session.turn('"FixtureKnowledge" "SecondEntity" "ThirdEntity" one two three');
...
test("real host/ACP controls save, repair overflow, enforce ownership and timeout, and persist across restart", { timeout: 30000 }, async () => {
  const provider = await startSessionControlProvider();
  const isolated = isolatedMemoryEnv({ NVIDIA_CHAT_COMPLETIONS_URL: provider.endpoint });
  let host = await startGuiHost({ env: isolated.env, port: 0, cwd: process.cwd() });
  let client = await WireClient.open(host.port);
  const other = await WireClient.open(host.port);
  let n = 0;
  try {
    client.send({ type: "session/new", requestId: "new", model: DEFAULT_MODEL_ID });
    ...
    async function save(budgets: Record<string, number>, instructions = identity) {
      return control({ action: "configureRuntime", revision: state.runtimePreferences.revision,
        settings: { instructions, budgets: { ...state.runtimePreferences.settings.budgets, ...budgets } } });
    }
    assert.equal((await save({ chatInputBytes: 16384, providerTimeoutMs: 1000 })).type, "session/control/ok");
    assert.equal(provider.requests.length, 0);
    other.send({ type: "session/control", requestId: "foreign", sessionId,
      control: { action: "configureRuntime", revision: state.runtimePreferences.revision, settings: defaults() } });
    ...
    client.send({ type: "prompt", requestId: "too-big", sessionId, text: "å".repeat(9400) });
    ...
    await save({ chatInputBytes: 65536, recentMessages: 0, semanticOutputTokens: 512 });
    client.send({ type: "prompt", requestId: "repaired", sessionId, text: "å".repeat(9400) });
    assert.equal((await client.until("prompt/ok", "repaired")).type, "prompt/ok");
    ...
    client.socket.close(); other.socket.close(); await host.close();
    host = await startGuiHost({ env: { ...isolated.env, A008_MEMORY_SQLITE_PATH: ":memory:" }, port: 0, cwd: process.cwd() });
