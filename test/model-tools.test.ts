...
import { mkdirSync, mkdtempSync, readFileSync, existsSync, rmSync, writeFileSync, symlinkSync } from "node:fs";
...
test("file tools preserve UTF-8/CRLF, reject stale and ambiguous edits, existing files and escaped paths", async () => {
  ...
  try {
    ...
    let id = 0;
    const execute = async (name: string, args: unknown) => JSON.parse(await port.execute({ id: String(++id), name, arguments: JSON.stringify(args) }));
    const content = "\uFEFFhej å🙂\r\nsecond\r\n";
    assert.equal((await execute("create_file", { path: "nested/new.txt", content })).status, "completed");
    const read = JSON.parse((await execute("read_file", { path: "nested/new.txt" })).text);
    assert.equal(read.content, content);
    const edit = { path: "nested/new.txt", expected_sha256: read.sha256, old_text: "second", new_text: "changed" };
    assert.equal((await execute("edit_file", edit)).status, "completed");
    assert.equal(readFileSync(join(cwd, "nested/new.txt"), "utf8"), content.replace("second", "changed"));
    assert.match((await execute("edit_file", edit)).text, /changed since/);
    assert.equal((await execute("create_file", { path: "nested/new.txt", content: "overwrite" })).status, "failed");
    const updated = JSON.parse((await execute("read_file", { path: "nested/new.txt" })).text);
    assert.match((await execute("edit_file", { ...edit, expected_sha256: updated.sha256, old_text: "\r\n", new_text: "\n" })).text, /exactly once/);
    assert.equal((await execute("read_file", { path: "../outside.txt" })).status, "failed");
    assert.equal((await execute("create_file", { path: ".git/config", content: "no" })).status, "failed");
    symlinkSync(outside, join(cwd, "linked"), process.platform === "win32" ? "junction" : "dir");
    assert.equal((await execute("create_file", { path: "linked/outside.txt", content: "no" })).status, "failed");
    assert.equal(existsSync(join(outside, "outside.txt")), false);
    const listed = JSON.parse((await execute("list_files", { path: ".", limit: 1 })).text);
    assert.equal(listed.entries.length, 1); assert.equal(listed.nextOffset, 1);
    assert.equal((await execute("git", { args: ["init", "--quiet"] })).status, "completed");
    const literal = "literal; name's.txt";
    await execute("create_file", { path: literal, content: "literal Git path" });
    assert.equal((await execute("git", { args: ["add", "--", literal] })).status, "completed");
    assert.match((await execute("git", { args: ["diff", "--cached", "--name-only"] })).text, /literal; name's.txt/);
    const tiny = await tools.prepare({ ...budgets, toolOutputBytes: 4 }, { approve: async () => true, update: async () => undefined }, new AbortController().signal);
    assert.equal(JSON.parse(await tiny.execute({ id: "bounded", name: "read_file", arguments: '{"path":"nested/new.txt"}' })).status, "failed");
  } finally { await tools.close(); rmSync(cwd, { recursive: true, force: true }); rmSync(outside, { recursive: true, force: true }); }
});

test("standalone GUI approval drives real file read/create/edit, literal Git and shell results back to the model", { timeout: 45000 }, async () => {
  let step = 0;
  const provider = await startSessionControlProvider(payload => {
    const observed = payload.messages.filter((m: any) => m.role === "tool");
    if (payload.messages.filter((m: any) => m.role === "user").at(-1).content.includes("DENY-WRITE")) {
      if (observed.length) {
        assert.match(observed.at(-1).content, /denied/);
        return { role: "assistant", content: "The write was denied; nothing created." };
      }
      return { role: "assistant", content: null, tool_calls: [{ id: "denied-write", type: "function", function: {
        name: "create_file", arguments: JSON.stringify({ path: "denied.txt", content: "must not exist" }),
      } }] };
    }
    const last = observed.at(-1);
    let name: string, args: unknown;
    switch (step++) {
      case 0: name = "read_file"; args = { path: "AGENTS.md" }; break;
      case 1: assert.match(last.content, /Fixture repository instructions/); name = "list_files"; args = { path: "." }; break;
      case 2: assert.match(last.content, /AGENTS.md/); name = "create_file"; args = { path: "hello.txt", content: "before\n" }; break;
      case 3: name = "read_file"; args = { path: "hello.txt" }; break;
      case 4:
        name = "edit_file"; args = { path: "hello.txt", expected_sha256: JSON.parse(JSON.parse(last.content).text).sha256, old_text: "before", new_text: "after" }; break;
      case 5: assert.match(last.content, /completed/); name = "git"; args = { args: ["status", "--short"] }; break;
      case 6: assert.match(last.content, /hello.txt/); name = "exec_command"; args = { cmd: process.platform === "win32" ? "Get-Content -LiteralPath hello.txt" : "cat hello.txt" }; break;
      default:
        assert.match(last.content, /after/);
        return { role: "assistant", content: "Verified file edit and Git status.", reasoning_content: "Transient fixture thought." };
    }
    return { role: "assistant", content: null, tool_calls: [{ id: `repo-${step}`, type: "function", function: { name, arguments: JSON.stringify(args) } }] };
  });
  const f = isolatedMemoryEnv({ NVIDIA_CHAT_COMPLETIONS_URL: provider.endpoint, PATH: process.env.PATH });
  const cwd = join(f.directory, "project"); mkdirSync(cwd);
  writeFileSync(join(cwd, "AGENTS.md"), "Fixture repository instructions.");
  execFileSync("git", ["init", "--quiet", cwd]);
  const host = await startGuiHost({ env: { ...f.env, A008_GUI_WORKSPACE: cwd }, port: 0 });
  ...
  try {
    ...
    assert.deepEqual(opened.state.runtime.tools.map((tool: any) => tool.name), ["exec_command", "list_files", "read_file", "create_file", "edit_file", "git"]);
    const sessionId = opened.sessionId;
    client.send({ type: "prompt", requestId: "work", sessionId, text: "Read AGENTS.md, list root, create and edit a file, inspect Git and verify it." });
    let approvals = 0;
    while (true) {
      ...
      if (frame.type === "tool/permission") {
        approvals++;
        if (frame.title === "create_file") assert.equal(existsSync(join(cwd, "hello.txt")), false);
        if (frame.title === "edit_file") assert.equal(readFileSync(join(cwd, "hello.txt"), "utf8"), "before\n");
        client.send({ type: "tool/permission", requestId: `allow-${approvals}`, sessionId, permissionId: frame.id, allow: true });
      }
      if (frame.type === "prompt/ok") {
        assert.equal(frame.state.messages.length, 2);
        assert.equal(frame.state.messages[1].content, "Verified file edit and Git status.");
        break;
      }
    }
    assert.equal(approvals, 7);
    assert.equal(readFileSync(join(cwd, "hello.txt"), "utf8"), "after\n");
    assert.equal(client.frames.filter(frame => frame.type === "tool" && frame.status === "completed").length, 7);
    const semantic = provider.requests.filter(p => String(p.messages.at(-1)?.content).includes('"operation"'));
    assert.ok(semantic.length > 0);
    assert.equal(JSON.stringify(semantic).includes("Fixture repository instructions"), false);
    assert.equal(JSON.stringify(semantic).includes("Transient fixture thought"), false);
    assert.equal(JSON.stringify(client.frames).includes(f.env.NVIDIA_API_KEY!), false);
    client.send({ type: "prompt", requestId: "deny", sessionId, text: "DENY-WRITE" });
    ...
    client.send({ type: "tool/permission", requestId: "reject", sessionId, permissionId: pending.id, allow: false });
    assert.equal((await client.until("prompt/ok", "deny")).state.messages.at(-1).content, "The write was denied; nothing created.");
    assert.equal(existsSync(join(cwd, "denied.txt")), false);
  } finally { client.socket.close(); await host.close(); await provider.close(); rmSync(f.directory, { recursive: true, force: true }); }
});
