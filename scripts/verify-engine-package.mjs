...
const packageRoot = resolve(process.argv[2] || "missing-package"),
  clientRoot = resolve(process.argv[3] || "missing-client");
...
try {
  Module._load = function (id, ...args) {
    return id === "electron"
      ? { app: {}, ipcMain: {}, BrowserWindow: { getAllWindows: () => [] } }
      : originalLoad.call(this, id, ...args);
  };
  ({ CoreRuntimeHost } = require(
    join(clientRoot, "electron/core-runtime-host.cjs"),
  ));
} finally {
  Module._load = originalLoad;
}
const { normalizeManifest } = require(
  join(clientRoot, "electron/brain-discovery.cjs"),
);
const manifest = normalizeManifest(
  JSON.parse(readFileSync(join(packageRoot, "agent007.brain.json"), "utf8")),
  join(packageRoot, "agent007.brain.json"),
);
assert.equal(
...
const command =
  process.platform === "win32"
    ? "Set-Content -LiteralPath 'package-proof.txt' -Value 'extracted engine'; Get-Content -LiteralPath 'package-proof.txt'"
    : "printf 'extracted engine' > package-proof.txt; cat package-proof.txt";
const provider = await startSessionControlProvider((payload) => {
  if (payload.messages.at(-1)?.role === "tool")
    return {
      role: "assistant",
      content: "Verified the extracted engine tool result.",
    };
  let user = payload.messages.at(-1)?.content || "";
  try {
    user = JSON.parse(user).message;
  } catch {}
  if (user === "PACKAGE-TOOL")
    return {
      role: "assistant",
      content: null,
      tool_calls: [
...
Object.assign(process.env, {
  NVIDIA_API_KEY: "synthetic-fixture-key",
  NVIDIA_CHAT_COMPLETIONS_URL: provider.endpoint,
  A008_SETTINGS_PATH: join(fixture, "settings.json"),
  A008_ENGINE_DATA_PATH: join(fixture, "engine-data"),
  A008_DEBUG_TRACE: "off",
  A008_ENGINE_LEGACY_CWD: "",
});
const host = new CoreRuntimeHost({ config: manifest.config });
const changes = new Set();
host.broadcast = (_channel, payload) => {
  for (const listener of changes) listener(payload);
};
function waitFor(check, timeoutMs = 15000) {
  return new Promise((resolve, reject) => {
    const timeout = setTimeout(() => {
      changes.delete(onChange);
      reject(new Error("Engine proof timed out."));
    }, timeoutMs);
    const onChange = () => {
      const value = check();
      if (value) {
...
try {
  ...
  const caps = await host.request("memory/capabilities", {
  ...
  const panel = host.status().panels[0];
  assert.equal(panel.sessionId, session.sessionId);
  ...
  assert.equal(attached.sessionId, session.sessionId);
  const run = host.request("session/prompt", {
    sessionId: session.sessionId,
    prompt: [{ type: "text", text: "PACKAGE-TOOL" }],
  });
  ...
  assert.equal(
    readFileSync(join(workspace, "package-proof.txt"), "utf8").trim(),
    "extracted engine",
  );
  assert.ok(
    provider.requests.some((p) =>
      p.messages.some(
        (m) => m.role === "tool" && m.content.includes("extracted engine"),
      ),
    ),
  );
  wire.send({
  ...
  assert.equal(
    ...
    "Verified the extracted engine tool result.",
  );
  const upload = await fetch(`${origin}/v1/upload`, {
    method: "POST",
    headers: {
      authorization: `Bearer ${access}`,
      "content-type": "application/octet-stream",
      "x-a008-filename": "package.txt",
    },
    body: "Synthetic package source.",
  });
  ...
  await host.request("session/prompt", {
    sessionId: session.sessionId,
    prompt: [{ type: "text", text: "After panel close" }],
  });
  ...
  console.log(
    "PASS: extracted portable engine → client discovery → ACP → shared panel/history → explicit approval → actual command/result → upload/memory → panel disconnect → graceful process stop.",
  );
} finally {
  wire?.socket.close();
  if (host.child) {
