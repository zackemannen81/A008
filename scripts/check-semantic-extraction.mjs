...
} from "../dist/src/orchestration/semantic-json-model.js";
import {
  PostOutputKnowledgeIntake,
  Utf8ByteKnowledgeIntakeMeasurer,
} from "../dist/src/orchestration/post-output-knowledge-intake.js";
...
if (!values.model)
  throw new Error("Specify --model; the check never silently switches models.");
const model = defaultModelRegistry.require(values.model).id;
const caps = generationCapabilities(model);
const generation = {
  ...SEMANTIC_JSON_GENERATION,
  ...(caps.topP ? {} : { topP: null }),
  maxTokens: Math.min(16384, caps.maxTokens),
};
const cases = [
  {
    id: "greeting",
    input: {
      ...
      answer: "Hej! Jag är redo att hjälpa till.",
    },
    expected: "Empty array; no social-exchange proposal.",
  },
  {
    id: "mixed",
    input: {
      message: "Hej! Jag föredrar svar på svenska.",
      answer: "Tack, då svarar jag på svenska.",
    },
    expected:
      "One language-preference claim, supported by the original message; no greeting proposal.",
  },
  {
    id: "source",
    input: {
      kind: "source",
      locator: "synthetic-fixture.txt",
      content: "The fixture service stores records in SQLite.",
    },
    expected: "One SQLite storage claim, with source support.",
  },
];
if (!values.live) {
  console.log(
    JSON.stringify(
      {
        ...
        maximumCalls: cases.length,
        retries: 0,
        perCallTimeoutMs: 180000,
        generation,
        instruction:
          "POST_OUTPUT_KNOWLEDGE_ANALYZER_INSTRUCTION in src/orchestration/semantic-json-model.ts",
        requests: cases,
      },
      null,
      2,
  ...
  if (!key)
    throw new Error(
      "NVIDIA_API_KEY is required; use the existing environment, never a command-line key.",
    );
  ...
  const identity = (kind) =>
    parseRuntimeId(
      `A008_v1_${kind}_85000000-0000-4000-8000-000000000001`,
      kind,
    );
  ...
  for (const fixture of cases) {
    try {
      const result = await intake.stage({
        ...fixture.input,
        taskId: identity("task"),
        applicabilityScopes: ["synthetic-check"],
        ...(fixture.input.kind === "source"
          ? { utteranceId: "synthetic-source-utterance" }
          : {}),
      });
      const proposals = result.proposals;
      const content = fixture.input.content ?? fixture.input.message;
      const supported = proposals.every(
        (p) =>
          p.support &&
          p.support.end > p.support.start &&
          p.support.start >= 0 &&
          p.support.end <= content.length &&
          p.support.source ===
            (fixture.input.kind === "source" ? "source" : "message"),
      );
      ...
      const meaning =
        fixture.id === "greeting"
          ? proposals.length === 0
          : proposals.length === 1 &&
            (fixture.id === "mixed" ? /svensk|swedish/iu : /sqlite/iu).test(
              proposals[0].proposal.proposition,
            );
      const passed =
        result.skippedProposals.length === 0 && supported && meaning;
      const summary = {
        case: fixture.id,
        strictJson: true,
        admittedProposals: proposals.length,
        skipped: result.skippedProposals.length,
        sourceSpansValid: supported,
        ...
        claims: proposals.map((p) => ({
          proposition: p.proposal.proposition,
          support: p.support,
        })),
      };
  ...
  const passed =
    results.length === cases.length && results.every((result) => result.passed);
  console.log(
    JSON.stringify({
      model,
      providerCalls,
