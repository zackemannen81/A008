...
export function createLocalAcpBridge(options: {
  ...
  const host = new EngineHost({ env: options.env, registry: options.registry,
    createPanels: false, resolveProject: () => project });
  const sessions = new Set<string>();
  const handlers = new Map<string, AcpPromptHandlers>();
  const permissions = new Map<string, { sessionId: string; resolve: (response: RequestPermissionResponse) => void }>();
  let closed = false;
  const requireSession = (id: string) => {
    if (closed || !sessions.has(id)) throw new Error("Unknown project session.");
  };
  const deny = (sessionId: string) => {
    for (const [id, permission] of permissions) if (permission.sessionId === sessionId) {
      permissions.delete(id); permission.resolve({ outcome: { outcome: "cancelled" } });
    }
  };
  const notify = async (message: SessionNotification) => {
    const current = handlers.get(message.sessionId); if (!current) return;
    const update = message.update;
    if ((update.sessionUpdate === "agent_message_chunk" || update.sessionUpdate === "agent_thought_chunk") && update.content.type === "text") {
      if (update.sessionUpdate === "agent_message_chunk") current.onAnswer(update.content.text);
      else current.onThought(update.content.text);
    }
    if (update.sessionUpdate === "tool_call" || update.sessionUpdate === "tool_call_update") {
      current.onTool?.({ type: "tool", sessionId: message.sessionId, id: update.toolCallId,
  ...
  return {
    async newSession(model) {
      ...
      const created = await host.newSession({ cwd: project.cwd, mcpServers: [] }, { notify,
        requestPermission: params => {
          const current = handlers.get(params.sessionId);
          if (!current?.onPermission) return Promise.resolve({ outcome: { outcome: "cancelled" } });
          const id = randomUUID();
          return new Promise(resolve => {
            permissions.set(id, { sessionId: params.sessionId, resolve });
            current.onPermission!({ type: "tool/permission", sessionId: params.sessionId, id,
              title: params.toolCall.title ?? "Tool execution", text: JSON.stringify(params.toolCall.rawInput ?? {}, null, 2) });
          });
        },
      });
      try {
        if (closed) throw new Error("Project bridge closed during session creation.");
        if (model !== undefined) host.control(created.sessionId, { action: "model", model });
        sessions.add(created.sessionId); return { sessionId: created.sessionId };
    ...
    cancel(id) { requireSession(id); deny(id); host.sessionAgent(id).cancel({ sessionId: id }); },
    async closeSession(id) { requireSession(id); deny(id); await host.closeSession(id); sessions.delete(id); },
    async inspectMemory(query) { if (closed) throw new Error("Project bridge is closed."); return project.runtime.inspectMemory(query); },
    async ingestSource(input) { if (closed) throw new Error("Project bridge is closed."); return project.agent.ingestSource(input); },
    async close() {
      if (closed) return; closed = true;
      for (const id of sessions) deny(id);
      await host.close(); sessions.clear(); handlers.clear();
    },
  };
}
