...
export class RuntimePreferencesStore {
  ...
  constructor(env: NodeJS.ProcessEnv, providerTimeoutMs: number) {
    ...
    this.path =
      configured === ":memory:"
        ? null
        : resolve(configured || join(homedir(), ".a008", "settings.json"));
    if (this.path !== null) {
      ...
      if (within === "" || (!within.startsWith("..") && !isAbsolute(within))) {
        throw new ChatError(
          "configuration",
          "A008_SETTINGS_PATH must be outside the A008 repository.",
        );
      }
    }
  ...
  snapshot(): RuntimePreferencesSnapshot {
    ...
    if (this.path !== null && existsSync(this.path)) {
      try {
        ...
        if (stored.version === 1) {
          ...
          if (
            !legacy?.budgets ||
            Object.keys(legacy.budgets).length !==
              RUNTIME_BUDGET_FIELDS.length - added.length ||
            added.some((k) => k in legacy.budgets)
          )
            throw new Error("legacy budgets");
          settings = parseRuntimePreferences({
            ...legacy,
            budgets: {
              ...Object.fromEntries(
                added.map((k) => [k, DEFAULT_RUNTIME_BUDGETS[k]]),
              ),
              ...legacy.budgets,
            },
    ...
    return {
      revision: digest(raw),
      settings: parseRuntimePreferences(settings),
      defaults: parseRuntimePreferences(this.#defaults),
      fields: RUNTIME_BUDGET_FIELDS.map((f) => ({ ...f })),
    ...
    try {
      if (this.path !== null) {
        ...
        try {
          ...
          throw new ChatError(
            "configuration",
            "Global settings are being saved by another process. Retry after it finishes.",
          );
        }
      }
      ...
      if (
        typeof value === "object" &&
        value !== null &&
        !("memoryLifecycle" in value)
      )
        settings = parseRuntimePreferences({
          ...settings,
          memoryLifecycle: current.settings.memoryLifecycle,
        });
      const suppliedPolicy = (value as { memoryLifecycle?: object })
        .memoryLifecycle;
      if (suppliedPolicy && !("association" in suppliedPolicy)) {
        settings = parseRuntimePreferences({
          ...settings,
          memoryLifecycle: {
            ...settings.memoryLifecycle,
            association: current.settings.memoryLifecycle!.association,
          },
        });
      }
      if (current.revision !== revision) {
        throw new ChatError(
          "configuration",
          "Global settings changed elsewhere. Reload saved settings before saving again.",
        );
      }
  ...
  get current(): RuntimePreferences {
    return this.#operation.getStore() ?? this.snapshot().settings;
  }
  run<T>(operation: () => T): T {
