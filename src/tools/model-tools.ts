...
export function toolEnvironment(env: NodeJS.ProcessEnv): Record<string, string> {
  ...
  const safe = new Set(["PATH", "PATHEXT", "SYSTEMROOT", "WINDIR", "COMSPEC", "TEMP", "TMP", "USERPROFILE", "HOME", "LANG", "LC_ALL", "APPDATA", "LOCALAPPDATA"]);
  return Object.fromEntries(Object.entries(env).filter((entry): entry is [string, string] => safe.has(entry[0].toUpperCase()) && typeof entry[1] === "string"));
}

export function boundedToolText(text: string, maximum: number): { text: string; truncated: boolean } {
  ...
  return { text: decoder.decode(bytes.subarray(0, maximum), { stream: true }), truncated: true };
}
...
export class ModelToolSession {
  ...
  constructor(options: { cwd: string; env: NodeJS.ProcessEnv; mcpServers?: readonly McpServer[] }) {
    this.#cwd = options.cwd;
    this.#env = options.env;
    this.#servers = options.mcpServers ?? [];
    if (this.#servers.some(server => "type" in server)) throw new Error("A008 supports approved stdio MCP servers only.");
    this.#register({ name: "exec_command", description: `Run a local ${process.platform === "win32" ? "Windows PowerShell (no profile)" : "POSIX shell"} command in ${this.#cwd}. Explicit user approval is required. Commands can read or change host files; this is not a filesystem sandbox. Use actual tool results, never claim execution from prose.`,
      parameters: { type: "object", properties: { cmd: { type: "string", minLength: 1 } }, required: ["cmd"], additionalProperties: false } },
    async (args, signal, budgets) => {
      const result = await runTerminalCommand({ command: args.cmd as string, cwd: this.#cwd,
  ...
  #register(definition: ChatToolDefinition, run: RegisteredTool["run"]) {
    const validate = new AjvJsonSchemaValidator().getValidator<Record<string, unknown>>(definition.parameters as JsonSchemaType);
    this.#tools.set(definition.name, { definition, validate, run });
  }

  async prepare(budgets: RuntimeBudgets, approval: ToolApproval, signal: AbortSignal): Promise<ChatTools> {
    ...
    if (!this.#ready) {
      try {
        for (const [index, server] of this.#servers.entries()) {
          ...
          const transport = new StdioClientTransport({ command: stdio.command, args: stdio.args, cwd: this.#cwd,
            env: { ...toolEnvironment(this.#env), ...Object.fromEntries(stdio.env.map(item => [item.name, item.value])) }, stderr: "ignore",
          ...
          await client.connect(transport, { signal, timeout: budgets.toolTimeoutMs });
          let cursor: string | undefined;
          const cursors = new Set<string>();
          do {
            const result = await client.listTools(cursor ? { cursor } : {}, { signal, timeout: budgets.toolTimeoutMs });
            for (const tool of result.tools) {
              if (this.#tools.size >= budgets.maximumToolDefinitions) throw new Error("MCP catalog exceeds Available tools budget.");
              const name = `mcp_${index}_${createHash("sha256").update(tool.name).digest("hex").slice(0, 16)}`;
              if (this.#tools.has(name)) throw new Error("MCP server returned duplicate tools.");
              this.#register({ name, description: `${stdio.name}: ${tool.name}. ${tool.description ?? ""}`, parameters: tool.inputSchema },
                async (args, callSignal, limits) => {
                  const result = await client.callTool({ name: tool.name, arguments: args }, undefined, { signal: callSignal, timeout: limits.toolTimeoutMs });
                  return { failed: result.isError === true, text: JSON.stringify(result) };
                });
            }
            cursor = result.nextCursor;
            if (cursor && cursors.has(cursor)) throw new Error("MCP server repeated its catalog cursor.");
    ...
    return { definitions: [...this.#tools.values()].map(tool => tool.definition), maximumCalls: budgets.maximumToolCalls,
      execute: async (call, callSignal) => this.#execute(call, budgets, approval, callSignal ?? signal) };
  }

  async #execute(call: ChatToolCall, budgets: RuntimeBudgets, approval: ToolApproval, signal: AbortSignal) {
    signal.throwIfAborted();
    const tool = this.#tools.get(call.name);
    if (!tool) throw new Error("Unknown tool.");
    const activity: ToolActivity = { id: call.id, name: call.name, cwd: this.#cwd, input: call.arguments, status: "pending" };
    ...
    try {
      ...
      await approval.update({ ...activity, status: "failed", output: "Invalid tool arguments; nothing executed." });
      return JSON.stringify({ status: "invalid_arguments", text: "Arguments do not match the offered tool schema. Nothing executed." });
    }
    ...
    signal.throwIfAborted();
    if (!allowed) {
      await approval.update({ ...activity, status: "failed", output: "User denied execution." });
      return JSON.stringify({ status: "denied", text: "User denied execution. Do not retry this action through another tool." });
    }
    ...
    try {
      ...
      const text = JSON.stringify({ status: result.failed ? "failed" : "completed", ...bounded });
      await approval.update({ ...activity, status: result.failed ? "failed" : "completed", output: text });
      return text;
    } catch (error) {
