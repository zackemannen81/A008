...
export function defaultSecretsPath(env: NodeJS.ProcessEnv = process.env): string {
...
export function loadProviderSecrets(path: string): ProviderSecrets {
  ...
  try {
    ...
    throw new ChatError("configuration", "Cannot read the provider secrets file.");
  }
}

export function saveProviderSecrets(path: string, secrets: ProviderSecrets): void {
...
export function resolveNvidiaApiKey(env: NodeJS.ProcessEnv, secretsPath: string): string | undefined {
