...
export const DEFAULT_MEMORY_LIFECYCLE_POLICY: MemoryLifecyclePolicy =
  Object.freeze({
    ...
    association: Object.freeze({
      strength: 0.4,
      halfLifeSeconds: 45 * 86400,
      threshold: 0.2,
      boost: 0.2,
    }),
    critical: Object.freeze({
      strength: 1,
      halfLifeSeconds: 365 * 86400,
      threshold: 0.2,
      boost: 0.2,
    }),
...
export function isKnowledgeSeverity(
  value: unknown,
): value is KnowledgeSeverity {
...
export function parseMemoryLifecyclePolicy(
  ...
  if (
    !raw ||
    raw.version !== "exponential-v1" ||
    ![
      "critical,important,minor,version",
      "association,critical,important,minor,version",
    ].includes(Object.keys(raw).sort().join())
  ) {
    throw new Error("Invalid memory lifecycle policy");
  }
  const category = (
    input: CreationLifecyclePolicy,
  ): CreationLifecyclePolicy => {
    if (
      !input ||
      Object.keys(input).sort().join() !==
        "boost,halfLifeSeconds,strength,threshold" ||
      !Number.isFinite(input.halfLifeSeconds) ||
      input.halfLifeSeconds <= 0 ||
      !Number.isFinite(Math.LN2 / input.halfLifeSeconds) ||
      Math.LN2 / input.halfLifeSeconds === 0 ||
      ![input.strength, input.threshold, input.boost].every(
  ...
  return Object.freeze({
    version: raw.version,
    critical: category(raw.critical),
    important: category(raw.important),
    minor: category(raw.minor),
    association: category(
      "association" in raw
        ? raw.association
        : DEFAULT_MEMORY_LIFECYCLE_POLICY.association,
    ),
  });
}
