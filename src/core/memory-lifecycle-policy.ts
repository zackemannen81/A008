...
export function parseMemoryLifecyclePolicy(value: unknown): MemoryLifecyclePolicy {
  ...
  if (!raw || raw.version !== "exponential-v1" || Object.keys(raw).sort().join() !== "critical,important,minor,version") {
  ...
  return Object.freeze({ version: raw.version, critical: category(raw.critical), important: category(raw.important), minor: category(raw.minor) });
}
