...
export const RUNTIME_BUDGET_FIELDS: readonly RuntimeBudgetField[] = Object.freeze([
  field("chatInputBytes", "Chat input budget", "UTF-8 bytes", "Complete serialized messages: system instructions, recent dialogue, retrieved memory and your message. Separate from generated tokens."),
  field("memoryProjectionBytes", "Memory projection budget", "UTF-8 bytes", "Retrieved memory selected for a chat turn. Omitted records remain stored; one oversized record can exceed this target."),
  field("recentMessages", "Recent dialogue", "messages", "Prior user and assistant messages sent with each question. 2 means one pair; 0 sends none. Stored conversation history is preserved.", 0),
  field("semanticInputBytes", "Semantic input budget", "UTF-8 bytes", "Complete request for retrieval scope, knowledge extraction and relation classification."),
  field("semanticOutputTokens", "Semantic output budget", "tokens", "Requested tokens for memory JSON. Each call uses the lower of this value and its model's supported output limit."),
  field("stagingBytes", "Extracted batch budget", "UTF-8 bytes", "Serialized proposals accepted from one extraction."),
  field("maximumProposals", "Proposals per extraction", "proposals", "Each proposal can require a separate classification call."),
  field("maximumTagsPerProposal", "Tags per proposal", "tags", "Maximum labels accepted on each extracted proposal."),
  field("maximumDomainsPerProposal", "Domains per proposal", "domains", "Maximum subject areas accepted on each extracted proposal."),
  field("maximumEntitiesPerProposal", "Entities per proposal", "entities", "Maximum entity labels accepted on each extracted proposal."),
  field("maximumScopeDomains", "Discussion scope", "domains", "Maximum accumulated domains per conversation. Least recently reinforced domains are evicted."),
  field("retrievalVocabulary", "Offered vocabulary", "labels/axis", "Maximum stored domains and tags offered to the retrieval scope classifier."),
  field("retrievalEntities", "Retrieval entities", "entities", "Maximum entity hints extracted from the current message."),
  field("retrievalTerms", "Retrieval terms", "terms", "Maximum lexical terms in the retrieval plan."),
  field("retrievalHistoryMessages", "Retrieval planning history", "messages", "Maximum recent messages used by the planner, within Recent dialogue.", 0),
...
export function parseRuntimePreferences(value: unknown): RuntimePreferences {
  ...
  for (const field of RUNTIME_BUDGET_FIELDS) {
    ...
    if (typeof n !== "number" || !Number.isSafeInteger(n) || n < field.minimum || n > field.maximum) {
      throw new ChatError("configuration", `${field.label} must be a whole number between ${field.minimum} and ${field.maximum} ${field.unit}.`);
    }
    budgets[field.key] = n;
  }
