...
export function prepareAssociations(
  ...
  const associationContext: AssociationClassifierContext | undefined = source === undefined ? undefined : {
    source,
    entities: [...entities].map(([handle, entity]) => ({ handle, labels: entity.labels, type: entity.type })),
    existing: context.relations instanceof RelationIndex ? context.relations.associationSnapshot().records.flatMap(edge => {
      const fromHandle = byId.get(edge.from), toHandle = byId.get(edge.to);
      return fromHandle && toHandle ? [{ fromHandle, toHandle, relation: edge.relation, scope: edge.scope }] : [];
    }) : [],
  };
  ...
  return {
    ...
    apply(value: unknown, proposalId: string, utteranceId: string, occurrenceId: string, at: string, policy: MemoryLifecyclePolicy): readonly string[] {
      if (value === undefined) return [];
      if (!Array.isArray(value)) return ["skipped_invalid_associations"];
      if (!(context.relations instanceof RelationIndex)) return value.map(() => "skipped_unavailable_index");
      const index = context.relations;
      const currentClaims = new Map<string, Claim>(context.evidence.listClaims().map(c => [c.id, c]));
      const currentEntities = new Map<string, Entity>(context.entities.list().map(e => [e.id, e]));
      const resolve = (handle: unknown): string | undefined => {
        if (handle === "proposal") return currentClaims.has(proposalId) ? proposalId : undefined;
      ...
      const artifact = context.evidence.listArtifacts().find(a => a.id === utterance?.artifactId);
      return value.map(item => {
        if (!item || typeof item !== "object" || typeof item.relation !== "string" || !item.relation.trim() || item.relation.length > 128) return "skipped_invalid_relation";
        const from = resolve(item.fromHandle), to = resolve(item.toHandle);
        if (!from || !to) return "skipped_unresolved_edge";
        const proof = item.support;
        if (!source || item.supportsRelation !== true || !proof || proof.source !== source.origin ||
            !Number.isSafeInteger(proof.start) || !Number.isSafeInteger(proof.end) || proof.start < 0 || proof.end <= proof.start || proof.end > source.content.length ||
            utterance?.content !== source.content || artifact?.locator !== locator) return "skipped_unproven_edge";
        return index.establishAssociation({ from, to, relation: item.relation, scope }, {
          occurrenceId, at, support: { utteranceId, start: proof.start, end: proof.end },
        }, policy);
      });
    },
  };
