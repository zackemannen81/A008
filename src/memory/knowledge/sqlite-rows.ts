...
function projectSnapshot(
  ...
  const insertBinding = table(
    ...
    ["namespace", "slot_key", "claim_id", "valid_from"],
  );
  const insertSlotIndex = table(
  ...
  for (const binding of snapshot.state.bindings) {
    ...
    const value = binding.kind === "attribute" ? binding.value : binding.object;
    insertBinding.add(
      namespace,
      key,
      binding.claimId,
  ...
  const insertLifecycle = table(
    "A008_knowledge_lifecycle",
    ["namespace", "evidence_id", "evidence_kind", "payload_json"],
    ["namespace", "evidence_id"],
  );
  const insertReceipt = table(
    "A008_knowledge_reinforcement_receipts",
    ["namespace", "occurrence_id", "evidence_id", "payload_json"],
    ["namespace", "occurrence_id", "evidence_id"],
  );
  for (const receipt of snapshot.lifecycle.receipts ?? [])
    insertReceipt.add(
      namespace,
  ...
  const insertLifeTransition = table(
  ...
  const insertRelation = table(
    "A008_knowledge_relations",
    ["namespace", "from_id", "to_id", "relation"],
    ["namespace", "from_id", "to_id", "relation"],
  );
  for (const link of snapshot.relations) {
    insertRelation.add(namespace, link.from, link.to, link.relation);
  }
  const associations = snapshot.associations ?? EMPTY_ASSOCIATIONS;
  new AssociationLifecycle().hydrate(associations);
  const insertAssociation = table(
    "A008_knowledge_association_lifecycle",
    ["namespace", "edge_key", "payload_json"],
    ["namespace", "edge_key"],
  );
  for (const record of associations.records)
    insertAssociation.add(namespace, record.key, JSON.stringify(record));
  const insertAssociationReceipt = table(
    "A008_knowledge_association_receipts",
    ["namespace", "occurrence_id", "edge_key", "payload_json"],
    ["namespace", "occurrence_id", "edge_key"],
  );
  for (const receipt of associations.receipts)
    insertAssociationReceipt.add(
      namespace,
      receipt.occurrenceId,
      receipt.edgeKey,
      JSON.stringify(receipt),
    );
  const insertAudit = table(
    "A008_knowledge_association_transitions",
    ["namespace", "seq", "payload_json"],
    ["namespace", "seq"],
  );
  associations.transitions.forEach((transition, index) =>
    insertAudit.add(namespace, index + 1, JSON.stringify(transition)),
  );

  const insert = table(
    "A008_knowledge_fts",
    ["namespace", "record_kind", "record_id", "labels"],
    ["namespace", "record_kind", "record_id"],
  );
  for (const entity of snapshot.entities) {
    insert.add(namespace, "entity", entity.id, entity.labels.join(" "));
  }
  for (const slot of snapshot.slots) {
...
export function insertKnowledgeSnapshot(
...
function sameRows(left: Row[], right: Row[]): boolean {
  ...
  if (left.length === 1)
    return left[0]!.every((value, i) => value === right[0]![i]);
...
export function updateKnowledgeSnapshot(
  database: BetterSqliteDatabase,
  namespace: string,
  before: KnowledgeNamespaceSnapshot,
  after: KnowledgeNamespaceSnapshot,
): void {
  const previous = projectSnapshot(namespace, before);
  const next = projectSnapshot(namespace, after);
  const deltas = next.map((table, i) => ({
    table,
    oldRows: groups(previous[i]!),
    newRows: groups(table),
  }));
  ...
  for (let i = next.length - 1; i >= 0; i--) {
    ...
    for (const [key, rows] of oldRows) {
      ...
      remove.run(
        ...table.keys.map((column) => rows[0]![table.columns.indexOf(column)]!),
      );
    }
  }
  ...
  for (let i = 0; i < next.length; i++) {
    ...
    const mutable = table.columns.filter(
      (column) => !table.keys.includes(column),
    );
    const fts = table.name === "A008_knowledge_fts";
    const upsert = fts
      ? insertSql(table)
      : `${insertSql(table)} ON CONFLICT(${table.keys.join(", ")}) DO ${mutable.length ? `UPDATE SET ${mutable.map((column) => `${column} = excluded.${column}`).join(", ")}` : "NOTHING"}`;
    ...
    for (const [key, rows] of newRows) {
      ...
      if (!fts && rows.length !== 1)
        throw new Error(`Duplicate knowledge key in ${table.name}`);
      if (fts && old) {
        ...
        database
          ...
          .run(
            ...table.keys.map(
              (column) => rows[0]![table.columns.indexOf(column)],
            ),
          );
      }
