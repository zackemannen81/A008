...
export const v2IdSchema = z
  ...
  .max(160)
  .regex(/^[A-Za-z0-9_-]+$/u);
export const v2ProjectIdSchema = z
  .string()
  .regex(
...
export const v2TicketRequestSchema = z
  ...
  .strict();
export const v2TicketResponseSchema = z.object({
...
export const v2AuthenticateSchema = z
  .object({
...
export const v2InfoSchema = z.object({
  ...
  authProfiles: z.array(z.enum(["device", "browser-pin"])),
  features: z.array(z.string()),
  limits: z.object({
    ticketLifetimeMs: z.number().int(),
    preauthFrameBytes: z.number().int(),
    authenticationDeadlineMs: z.number().int(),
...
export const v2DeviceGrantSchema = z
  .object({
    ...
    projects: z.array(v2ProjectIdSchema).min(1).max(100),
    capabilities: z.array(v2CapabilitySchema).min(1).max(8),
...
export function v2AuthOpenApiDocument() {
  ...
  const errors = Object.fromEntries(
    [400, 401, 403, 404, 415, 429, 500].map((status) => [
      status,
      { description: "Structured V2 error", content: json("v2-error") },
    ]),
  );
  return {
    openapi: "3.1.1",
    info: { title: "A008 V2 authentication", version: "2.0.0" },
    paths: {
      "/v2/info": {
        get: {
          security: [],
          responses: {
            200: {
              description: "Public protocol discovery",
              content: json("v2-info"),
            },
            ...errors,
          },
        },
      ...
      "/v2/auth/ticket": {
        post: {
          ...
          requestBody: { required: true, content: json("v2-ticket-request") },
          responses: {
            200: {
    ...
    components: {
      schemas: v2AuthJsonSchemas(),
      securitySchemes: {
        device: { type: "http", scheme: "bearer" },
        browserPin: { type: "apiKey", in: "cookie", name: "a008_auth" },
      },
    },
  };
}
