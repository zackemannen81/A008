import { memorySnapshotSchema, type MemorySnapshot, type FrameCheck } from './http-schemas.js';
export function parseMemorySnapshot(value: unknown): MemorySnapshot {
  if (!memorySnapshotSchema.safeParse(value).success) throw new Error('The host returned an incompatible memory response. Restart the host with the current A008 build.');
...
export function parseFrameCheck(value: unknown, url: string): FrameCheck {
  ...
  return { url: typeof payload.url === 'string' ? payload.url : url,
