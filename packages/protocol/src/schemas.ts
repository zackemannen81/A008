...
export const sessionParametersSchema = z.object({
  stream: z.boolean(),
  temperature: finite.nullable(),
  topP: finite.nullable(),
  maxTokens: finite,
  enableThinking: z.boolean().nullable(),
  reasoningBudget: finite.nullable(),
  reasoningEffort: text.nullable(),
  seed: finite.nullable(),
  stop: z.array(text).readonly().nullable(),
});
...
export const generationCapabilitiesSchema = z.object({
  ...
  thinking: z.boolean(),
  reasoningBudget: finite.nullable(),
  reasoningEfforts: z.array(text).readonly(),
  seed: z.boolean(),
  stop: z.boolean(),
  verifiedOn: text,
});
export type GenerationCapabilities = z.infer<
  typeof generationCapabilitiesSchema
>;
...
export const modelSchema = z.object({
  ...
  name: text,
  provider: nonempty,
  executionProvider: nonempty,
  inputModalities: z.array(modelModalitySchema).min(1).readonly(),
  verifiedOn: nonempty.optional(),
  defaults: sessionParametersSchema,
  capabilities: generationCapabilitiesSchema,
});
...
export const runtimePreferencesSnapshotSchema = z
  .object({
    revision: text,
    settings: runtimePreferencesSchema,
    defaults: runtimePreferencesSchema,
    fields: z.array(runtimeBudgetFieldSchema).min(1).readonly(),
    storagePath: text.nullable(),
  })
  .superRefine((snapshot, ctx) => {
    const keys = new Set(snapshot.fields.map((field) => field.key));
    const invalid =
      keys.size !== snapshot.fields.length ||
      snapshot.fields.some((field) => field.maximum < field.minimum) ||
      [snapshot.settings, snapshot.defaults].some(
        (settings) =>
          Object.keys(settings.budgets).length !== keys.size ||
          snapshot.fields.some((field) => {
    ...
    if (invalid)
      ctx.addIssue({
        code: "custom",
        message: "Runtime budgets must match their declared fields and bounds.",
      });
  });
...
export type SessionSnapshot<R = RuntimePreferencesSnapshot> = Omit<
  z.infer<typeof sessionSnapshotSchema>,
  "runtimePreferences"
> & { readonly runtimePreferences?: R };
...
export const sessionControlInputSchema = z.discriminatedUnion("action", [
  ...
  z.object({
    action: z.literal("configureRuntime"),
    settings: z.unknown(),
    revision: text,
  }),
]);
...
export type SessionControl =
  | Exclude<SessionControlInput, { action: "configure" | "configureRuntime" }>
...
export const clientMessageSchema = z.discriminatedUnion("type", [
  ...
  z.object({
    ...
    requestId: commandId,
    sessionId: commandId,
    control: sessionControlInputSchema,
  }),
]);
type WithControl<T, C> = T extends { control: unknown }
  ? Omit<T, "control"> & { control: C }
  : T;
export type ClientMessage<C = SessionControl> = WithControl<
  z.infer<typeof clientMessageSchema>,
  C
>;
...
const replies = [
  z.object({
    type: z.literal("session/resume/ok"),
    requestId: nonempty,
    sessionId: nonempty,
    resumeToken,
    state: sessionSnapshotSchema.optional(),
...
export const hostServerMessageSchema = z.discriminatedUnion("type", [
  newSession.required({ resumeToken: true }),
  ...replies,
]);
type WithState<T, S> = T extends { state: unknown }
  ? Omit<T, "state"> & { state: S }
  : "state" extends keyof T
    ? Omit<T, "state"> & { state?: S }
    : T;
export type ServerMessage<S = SessionSnapshot> = WithState<
  z.infer<typeof serverMessageSchema>,
  S
>;
export type HostServerMessage<S = SessionSnapshot> = WithState<
  z.infer<typeof hostServerMessageSchema>,
  S
>;

export function isParameters(value: unknown): value is SessionParameters {
  return sessionParametersSchema.safeParse(value).success;
}
export function isRuntimePreferencesSnapshot(
  value: unknown,
): value is RuntimePreferencesSnapshot {
  return runtimePreferencesSnapshotSchema.safeParse(value).success;
}
export function parseSessionSnapshot(value: unknown): SessionSnapshot {
  if (!sessionSnapshotSchema.safeParse(value).success)
    throw new Error("Host sent an invalid session snapshot.");
  return value as SessionSnapshot;
}
export const v1JsonSchemas = () =>
  Object.fromEntries(
    Object.entries({
      clientMessage: clientMessageSchema,
      serverMessage: serverMessageSchema,
      hostServerMessage: hostServerMessageSchema,
