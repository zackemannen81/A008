...
- Phase: repository bootstrap; no a007 product source exists yet.
- License: Apache-2.0 for a007-owned repository contents. Imported or adapted
...
- `C:\code\OpenHands` is an external MIT-licensed Agent Canvas source clone.
  Its repository instructions govern changes made there; it is not a007 truth.
- The a007 memory-engine has not been created. The owner-supplied Context-First
...
- Bootstrap and protocol/add-on copies in the repository root are ignored
  reference inputs, not shipped a007 content.
...
- Writing clones live under `C:\code\a007-workers`, never inside the canonical
  `C:\code\a007` working tree.
...
- Do not execute the legacy client. Its exposed credential was revoked and
  ...
  remains unsafe provenance. Use the new environment-backed a007 adapter.
