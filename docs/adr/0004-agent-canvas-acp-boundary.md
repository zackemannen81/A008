...
stdio. a007 must reuse its existing provider-neutral core and NVIDIA adapter,
...
- Agent Server launches the `a007-acp` executable as a Custom stdio agent. No
  OpenHands repository is modified or vendored for this boundary.
- `a007-acp` implements stable ACP v1 with the exact
...
- Agent Server owns subprocess lifetime and its own external conversation
  record. The a007 ACP session ID and history are process-local and ephemeral.
...
- OpenHands Agent Canvas was inspected at
  ...
  copied and it is not an a007 package dependency.
...
renderer-facing system, and creates a second provider owner beside a007.

## Consequences

- Canvas can select a007 through its existing Custom ACP settings and render
  the standard events without an a007-specific frontend fork.
