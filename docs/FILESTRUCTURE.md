...
A008-0177 adds src/runtime/project-workspace-store.ts, the local SQLite and literal-Git owner for shared/worktree project session metadata and clean-only discard. est/project-workspace-store.test.ts uses a temporary Git repository to verify isolation lifecycle behavior.
