...
- [x] Packed `@a008/client` + `@a008/protocol` install outside A008; independent TypeScript consumer compiles and runs.
- [x] Client package source imports only `./` and `@a008/protocol`; no Node/React/runtime/GUI/DOM-global coupling.
- [x] Cookie, bearer and engine credential adapters tested separately.
- [x] V2 SDK client authenticates, opens a session, prompts, inspects snapshot/events and refuses unknown-command replay against a real host.
- [x] Bundled GUI session, upload, memory, shell, projects and settings tests pass through the SDK.
- [x] Root typecheck/build, GUI typecheck, `test:core`, membership, GUI suite, `verify:protocol`, `verify:client` and `git diff --check` pass.
...
- [x] Charter frozen Ready after claim landed.
- [x] Implement `@a008/client` with V1/V2 adapters and credential adapters.
- [x] Migrate bundled GUI session/HTTP onto the SDK.
- [x] Independent consumer and import-boundary proofs.
- [x] Full verification and owning docs.
...
- Identity claimed on local main as `7a3eefb` then branched `grok/A008-0149-stage5-client-sdk`.
- `npm run verify:client`: PASS packed client+protocol+zod installed offline outside A008; independent TypeScript consumer compiled and ran.
- `npm run verify:protocol`: PASS.
- Focused client tests: 4/4 (adapters, no-replay, import boundary, real-host V2 prompt/inspect).
- Final `npm test`: 711 core + 4 membership + 187 GUI pass, 0 failures/skips.
- Root typecheck/build: pass.
- GUI typecheck and production build: pass. Existing Zod annotation and >500 kB chunk warnings remain (main JS 567.84 kB / 169.76 kB gzip).
- `git diff --check`: pass.
- No live/paid provider call.
- Skipped: live device-grant against a remote edge; Expo/native secure storage (Stage 6); switching bundled chat onto V2 WebSocket (accepted charter constraint).
...
- [x] `docs/CLIENT_API_V2.md` / `docs/CLIENT_AUTH.md`
- [x] `docs/tasks/A008-0103_stable-client-api-program.md`
- [x] `README.md` as needed
- [x] archive + handoff; journal remains the operator merge entry

## Handoff and Follow-ups

- Current state: Complete on `grok/A008-0149-stage5-client-sdk`. Stage 5 is complete pending integration.
- Next recommended step: after merge/journal, begin Stage 6 Expo/native proof or a bounded child that adds V2 prompt attachment/image commands if bundled chat should leave the V1 adapter.
- Blockers: none.
- Child tasks: none.
