...
- [x] `temperature: null` is treated as explicit omission by semantic JSON generation.
- [x] GPT-5.6 Luna semantic generation emits no `temperature` while preserving `reasoningEffort: "none"`.
- [x] ACME parity fixture proves both `retrieval_scope` and `knowledge_analysis` execute through ACME without `temperature`.
- [x] Focused semantic/ACME suite: 34/34 passed.
- [x] `npm run typecheck --silent` passed.
- [x] `git diff --check` passed.
- [x] Live GUI host rebuilt/restarted and post-output extraction produced committed knowledge.

## Outcome

Luna semantic retrieval classification and post-output knowledge analysis now use the provider default temperature. A008 remains owner of retrieval, extraction, reconciliation and memory semantics; ACME remains execution-only.
