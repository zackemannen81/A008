...
Status: Complete
Owner: ChatGPT (operator takeover after delegated attempts)
Created: 2026-09-17
...
- `entities[]` is now materialized as independent referents; co-mentioned labels are never aliases of the first item.
- Registry lookup deterministically normalizes lexical identity (`React`/`react`) while retaining preferred display spelling.
- Structural claim↔entity membership is stored separately from `RelationIndex` and projected as `entity_ref`.
- Current-batch entities receive stable classifier handles before persistence; structured propositions reach proposals and stored candidates.
- Statement slot ownership follows structured proposition semantics; unstructured fallback is proposition-specific, never `entities[0]`.
- Single and batch relation prompts compose one shared, self-contained relation/association contract; severity remains lifecycle-only.
- Inspection exposes stored/effective/primary-effective domains without persisting inherited domains onto entities.
- Dialogue answer-only discoveries use assistant utterance provenance and cannot pass user-assertion acceptance.
- Knowledge SQLite schema advances to 5 for structural claim/entity membership; no legacy backfill is required.

## Verification

- [x] Focused A008-0122 regressions: 7/7 pass.
- [x] `npm run test:core`: 644/644 pass, 0 failures/skips/cancellations/todo.
- [x] `npm run typecheck`: pass.
- [x] `npm run build`: pass (also executed by `test:core`).
- [x] Structural references survive SQLite close/reopen.
- [x] Current-batch semantic association endpoints resolve without creating structural L3 lifecycle.
- [x] Assistant-answer-only provenance remains assistant-attributed and unaccepted as user assertion.
- [x] No live provider call or legacy-store migration was required.

## Documentation Updates

- [x] `docs/CURRENT_STATUS.md` records the implemented 0122 contract and verification.
- [x] `docs/SYSTEMDOC.md` records structural references, classifier input/prompt parity, derived domains and dialogue origin.
- [x] `docs/FILESTRUCTURE.md` records new owners and regression coverage.
- [x] `docs/KNOWLEDGE_MEMORY_MODEL.md` clarifies co-mentioned referents and structural-vs-semantic association topology.
- [x] `docs/CURRENT_TASK.md` restored from the canonical template before final commit.
