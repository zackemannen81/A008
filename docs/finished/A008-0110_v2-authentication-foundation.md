...
## Mutable progress
- [x] Implement schemas, credentials, tickets and local CLI.
- [x] Integrate and verify host discovery/ticket endpoints.
- [x] Complete documentation, archive/template and authorized integration.

## Verification and final necessity review

Four new tests cover actual grant/list/revoke CLI, hash-only persistence,
restart/expiry, per-project/capability refusal, one-use ticket expiry/revocation,
capacity, real host Origin/auth/routes and configured PIN versus invalid Bearer.
Full npm test: 591 core + 4 membership + 161 GUI = 756, no failures/skips.
Root/GUI builds pass. GUI main chunk is 533.97 kB / 161.63 kB gzip; existing
size/dependency-annotation warnings remain. Independent installed protocol proof
now includes V2 auth schemas/OpenAPI. Portable engine proof passes with the
authentication guide bundled. No live providers, owner credentials or service
restart; fixtures use temporary stores and synthetic grants only.
Final review matches PC-01/05 and ADR 0041; only the scoped auth foundation is
exposed. Auth files contain hashes/metadata, not raw credentials. Discovery has
no project/host paths. V2 sessions and live revocation remain future work.
Docs links/fences, generated schemas, frozen plan hash, archive/template and
diff checks pass. Owner requested stopping at this task boundary; no next task
is allocated or started. Resume from the parent program and handoff.
