...
A008-0004 proved the custom ACP bridge at process level. This task proves the
next boundary through a real local OpenHands Agent Canvas and Agent Server: a
browser turn must reach the compiled A008 bridge and return a deterministic
...
Produce the first visible Agent Canvas chat result through the A008 ACP bridge.
...
`A008-acp` -> the existing NVIDIA adapter -> a loopback fake endpoint and back.

### In Scope

- Install the pinned OpenHands clone's local npm dependencies and the `uv`/`uvx`
  runtime prerequisite when absent.
- Add a deterministic A008-owned fake NVIDIA chat-completions server for local
  runtime verification only.
- Build A008 and start OpenHands Agent Canvas 1.16.0 plus Agent Server 1.44.1
...
- Configure the active Agent profile through the Canvas UI as Custom ACP with
  the compiled A008 bridge command.
...
- Update the owning A008 documentation, including the existing file-map defect
...
- Adopting OpenHands source into A008, desktop packaging, installer work,
...
- A clean install/build of A008 and the pinned OpenHands clone succeeds.
- The browser visibly renders the expected deterministic answer after a prompt
  sent through the configured custom A008 ACP command.
...
- Evidence, setup/operation instructions, limitations, status, system behavior,
  file map, and journal are durable in A008.
- The task is archived and `docs/CURRENT_TASK.md` is restored.

### Minimum Verification Gates

- [x] A008 clean install, typecheck, build, and all automated tests.
- [x] OpenHands clean npm install, application build, and source-tree check.
- [x] Real Canvas + Agent Server browser round trip through compiled `A008-acp`
...
- Worker path:
  `C:\code\A008-workers\A008-0005_agent-canvas-runtime-proof`.
- A008-0004 bridge task: `docs/finished/A008-0004_agent-canvas-acp-bridge.md`.
...
- OpenHands `docs/ACP_AGENTS.md` and its existing Custom ACP mock-LLM browser
  test are implementation references, not A008 authority.
...
- Observed: OpenHands already exercises Custom ACP configuration and a browser
  conversation in its own mock-LLM suite; the A008 proof substitutes only the
  compiled A008 ACP process and its loopback fake NVIDIA transport.
...
- Observed: the successful A008 model request reached only the loopback fixture,
...
- [x] `npm ci` installed five A008 packages and reported zero vulnerabilities;
...
- [x] The browser saved the active Custom ACP profile with a forward-slash
  ...
  `A008-CANVAS-LOOPBACK-OK`, and showed no error banner. Stable capture found no
