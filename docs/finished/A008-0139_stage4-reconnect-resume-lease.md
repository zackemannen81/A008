...
- [x] Real-host disconnect produces detached state and cancels active work/denies pending approval.
- [x] Same-principal/project/session/capability resume within lease succeeds once.
- [x] Wrong principal/project/session/capability and second writer fail.
- [x] Deterministic 45-second expiry and explicit close invalidate resume authority.
- [x] Resume returns authoritative snapshot/sequence and only later events are applied.
- [x] No thought replay, model regeneration, command replay or tool re-execution occurs.
- [x] Resume capability is absent from logs/snapshots/provider payloads.
- [x] Existing V1 reconnect semantics and full root/protocol gates remain green.
...
- [x] Confirm A008-0138 is Complete on integration main before implementation.
- [x] Implement detach/resume lifecycle and opaque capability.
- [x] Add lease/authority/snapshot/no-replay regressions.
...
- Resume capability is absent from ordinary snapshots and receipts; real-host tests prove no provider/tool replay after resume.
...
- Current state: Complete and verified on `chatGPT/A008-0139-reconnect-resume-lease`; ready for PR/integration.
- Next recommended step: after A008-0139 integrates Complete on main, activate A008-0140 restart uncertainty + Stage-4 closure proof.
- Blockers: none.
- Child tasks: none.
