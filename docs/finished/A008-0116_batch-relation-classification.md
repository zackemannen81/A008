...
- [x] Multi-proposal post-output cycle uses exactly two semantic model calls.
- [x] Batch handle validation rejects unknown/future/cross-proposal handles.
- [x] Existing-candidate relation decisions still reconcile correctly.
- [x] Earlier same-batch proposal handles resolve after earlier commits.
- [x] ACME route observes `knowledge_analysis`, then one `relation_classification`.
- [x] Existing relation-gated commit tests remain green.
...
- `git diff --check` — PASS.
- ACME parity fixture with three proposals observes exactly one `knowledge_analysis`
  and one `relation_classification`; retrieval remains a separate preceding call.
- Earlier same-batch proposal handles resolve through the sequential commit path;
  a future proposal handle fails before any claim is persisted.
- Existing single-decision semantic fixtures remain compatible only for a
  one-item batch; multi-item batches never fan back out into N provider calls.
- Implementation refinement: the live `KnowledgeEngineCommit` already compares
  against the project claim set rather than per-proposal indexed lookup, so the
  batch materializes that existing candidate surface once and keeps all mapping,
  acceptance, reconciliation, provenance and persistence inside A008.
