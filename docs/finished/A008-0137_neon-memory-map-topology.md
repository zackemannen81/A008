...
- [x] Graph rendering regression covers kind forms, arrows, relation labels, topology weight and focus context.
- [x] GUI typecheck/test/build pass, or unavailable gates are recorded with the actual cause.
- [x] `git diff --check` passes.
- [x] Final review confirms no memory/API/runtime behavior is changed.
...
- [x] Archive, hand off and restore the current-task template after integration via retrospective docs-first repair.
...
- Current state: complete and merged on main through PR #78; docs-first closure repaired retrospectively.
- Next recommended step: none for this task.
- Blockers: none.
- Child tasks: none.
