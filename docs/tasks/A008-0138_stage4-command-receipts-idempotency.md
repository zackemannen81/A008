...
- [ ] Shared protocol/schema round trips for command IDs, receipts, conflict/capacity/unknown outcomes.
- [ ] Real-host same-ID/same-payload retry proves one mutation/turn only.
- [ ] Real-host same-ID/different-payload retry returns conflict.
- [ ] Duplicate permission resolution cannot execute a tool twice.
- [ ] Running and retained terminal receipt lookup obeys original authority.
- [ ] Five-minute / 1,024-per-principal policy is deterministically testable without wall-clock sleeps.
- [ ] Capacity never evicts running or unexpired receipts.
- [ ] Secret-redaction review covers receipt/error output.
- [ ] Existing A008-0132 focused tests plus full root/protocol gates remain green.
...
- [ ] Implement shared command/receipt contract.
- [ ] Implement host-owned bounded receipt registry.
- [ ] Route covered V2 mutations through receipt semantics.
- [ ] Add focused real-host/idempotency regressions.
...
- A worker implementing this task must branch from current main, copy this charter into `CURRENT_TASK.md`, and revalidate the pinned contract if `PROJECT_BRIEF.md` or the accepted V2 decisions changed.
...
- [ ] `docs/SYSTEMDOC.md`
- [ ] `docs/CLIENT_API_V2.md` if implementation detail clarification is required without changing accepted semantics
...
- Current state: Ready; implementation not started.
- Next recommended step: implement A008-0138 before A008-0139.
- Blockers: none.
- Child tasks: none.
- Resume condition: current main still satisfies the accepted contract.
- Open questions: none.
