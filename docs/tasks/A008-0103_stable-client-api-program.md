...
A008-0114 completed Stage 3.5. The adapter exists as explicit local composition (`A008_CHAT_TRANSPORT=acme`) and does not call ACME `/v1/execute`. Direct-vs-ACME fixtures showed equivalent A008 chat/tool/semantic/knowledge outcomes from equivalent normalized model output, plus typed failure evidence and no post-dispatch fallback. The recorded adoption decision is **NO-GO**: frozen ACME `ModelRequest` cannot carry A008 thinking/reasoning/topP/seed controls, so ACME is not the normal execution substrate. Stage 4 starts without ACME and must keep application `commandId`/`turnId`/event sequence distinct from any later ACME execution identity.
