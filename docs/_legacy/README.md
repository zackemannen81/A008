...
The local `agenten007/` directory is owner-supplied evidence for the pre-A008
...
the directory: it still embeds the retired value. A008-0003 replaced secret
access with `NVIDIA_API_KEY` at the new adapter boundary and uses fake HTTP for
normal verification.

## Intake rule

A008-0003 extracted reviewed behavior into new A008-owned modules without
