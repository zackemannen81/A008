...
After authentication one socket is bound to one principal/project and at most one attached session. V2 commands are `session/new`, `session/inspect`, `session/prompt`, `session/cancel`, `session/control` and `tool/permission`. Project scope, device existence/expiry and `session` capability are rechecked on every operation. Foreign principal/project/session references and a second writer fail before runtime work. Concurrent session creation on one socket is fenced. A008-0138 requires `commandId` on every mutating command; `session/inspect` and `session/control` with `action=inspect` remain read-only and do not require one.

Revocation or credential expiry closes the live socket, cancels owned active work and resolves pending tool permissions denied. Tool permission IDs are one-use and connection/session-bound. Outgoing V2 text passes the existing credential redactor. A008-0132 defines event sequence/snapshot ordering and terminal turn outcomes. A008-0138 requires stable `commandId` on mutations, suppresses same-command re-execution through principal-scoped receipts and exposes authenticated `GET /v2/projects/{projectId}/commands/{commandId}`; unknown/expired/foreign receipts fail as `COMMAND_UNKNOWN`. V2 still does not claim reconnect/resume leases or restart uncertainty handling.
