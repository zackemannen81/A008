...
A008-0136 makes the implemented Stage-4 boundary discoverable rather than expanding it. Public `GET /v2/info` now advertises the A008-0132 identity/order/snapshot/terminal features, A008-0138 command receipts/idempotency, and A008-0139 `session.reconnect-resume`, with their bounded limits. The bundled GUI Parameters → Runtime view reports only restart uncertainty as the remaining Stage-4 gap. Bundled chat still uses the V1 WebSocket owner; migrating the web client to V2 remains Stage 5. The existing V1 45-second detached-session resume capability remains separate compatibility behavior.
