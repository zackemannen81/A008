...
## GUI file browser and basic editor

A008-0182 adds authenticated `GET /v1/files`, `GET /v1/file` and `POST /v1/file` host routes. The host owns all filesystem access within the current workspace: paths are relative and contained, `.git` and symbolic links are refused, text must be strict UTF-8 without NUL bytes, and each opened or saved file is capped at 256 KiB. Save requires the SHA-256 returned by the corresponding read; a stale revision is refused before writing. The browser has no filesystem or shell authority. `gui/src/files/` lists directories, opens eligible files and uses the existing escaped `HighlightedEditor`; unsupported language names and oversized highlighted content remain plaintext according to that existing highlighter contract. Binary preview, search, filesystem mutation other than guarded text replacement, tabs and file watching are not implemented.

## GUI sidebar controls and application menu

A008-0183 makes the desktop sidebar renderer-local and operator-controlled. Its width is stored locally, bounded from 208 to 480 pixels, and is changed by a keyboard-accessible separator with a `col-resize` pointer affordance. The header control hides or restores the sidebar without changing the mounted page; narrow-screen navigation retains its existing menu behavior. The header also has File, Edit, View and Help application menus that route only existing GUI actions: New chat, Files, Parameters, sidebar visibility and Help. This adds no native menu, host route, runtime/protocol behavior, font, colour token or theme change.
...
`src/platform/platform-store.ts` now persists a required `workspaceId` on every Platform V3 conversation and copies it as immutable provenance to every accepted run. Schema version 2 migrates version-1 rows to the explicit `legacy-unbound` marker; the coordinator refuses such a missing/unavailable workspace rather than falling back to a project root. `POST /v3/projects/:projectId/conversations` creates an isolated Git worktree through `GuiWorkspaceStore` before persisting the conversation and discards that new worktree if persistence fails. The coordinator resolves a queued run's workspace from the stored ID, opens the shared project runtime by project ID, and supplies the resolved worktree path as the EngineHost session CWD. Thus V3 client/window lifetime remains observation/control only: an accepted run remains coordinator-owned, and another client can inspect it after the submitter disconnects. The bundled Platform view exposes the conversation workspace ID. Existing V1 chat ownership is unchanged.
