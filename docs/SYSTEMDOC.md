...
A008-0136 makes the implemented Stage-4 boundary discoverable rather than expanding it. Public `GET /v2/info` now advertises the A008-0132 identity/order/snapshot/terminal features plus A008-0138 `session.command-receipts` and `session.command-idempotency`, with the five-minute/1,024 receipt limits. The bundled GUI Parameters → Runtime view reports reconnect/resume lease and restart uncertainty as the remaining Stage-4 gaps. Bundled chat still uses the V1 WebSocket owner; migrating the web client to V2 remains Stage 5. The existing V1 45-second detached-session resume capability is compatibility behavior and is not a V2 Stage-4 lease.
