...
A008-0136 established the typed bundled Zero Cost Radar projection; A008-0151 keeps that offline baseline and adds an explicit live update/import boundary. `GET /v1/catalog/zero-cost` returns the repository-owned A008-0134 `ZERO_COST_MODEL_ROUTES` snapshot without network access. `POST /v1/catalog/zero-cost` is the only live check: the host fetches ZeroCostRadar's published `data/a008-model-routes.json`, validates it with the shared `zeroCostCatalogSchema`, requires each route verification date to equal the catalog verification date, and returns the result only for that request. The remote feed is not persisted as A008 runtime truth and does not register a provider.

Parameters → Zero Cost starts from the bundled snapshot, exposes explicit Update check state, and computes availability from the current `/v1/models` result. A route already present in the shipped registry/user catalog is marked IN A008. One-click add is enabled only when the route matches A008's existing NVIDIA Chat Completions execution path; it reuses the existing host-owned user catalog mutation and then reloads `/v1/models`. Adding the same model id replaces the existing user-catalog row rather than duplicating it. OpenRouter, OpenCode, Groq and Google rows remain visible discovery evidence with an unsupported-provider reason until separate execution support exists. Refresh/import never changes the active/default model/provider, credentials, routing, or paid fallback.
