...
A008-0142 is active on its dedicated implementation branch after merging current `main`. Its frozen charter owns generated-image chronology: pending/completed/failed image output must be canonical ordered conversation content, manual and model-triggered generation must converge on one host/session-owned lifecycle, and provider output remains durable through the existing source store. The earlier renderer-local checkpoint is implementation evidence only; reload/reconnect ordering and the structured `generate_image` tool are not yet complete.
