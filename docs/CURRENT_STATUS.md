...
| Model registry | Two profiles. `nvidia/nemotron-3.5-lightning-30b-a3b` is the default, text-only, defaults checked 2026-09-01. `nvidia/Nemotron-3-Nano-Omni-30B-A3B-Reasoning-NVFP4` was added 2026-09-04 with the vendor's documented thinking-mode defaults; it declares text, image, video and audio input. Ids are matched exactly, case included, because the vendor publishes NVFP4, BF16 and FP8 as separate ids. `ModelProfile` now carries `inputModalities` and `verifiedOn`; a profile without a verification date is a guess and the suite fails on one. A008 still sends only text to a chat turn — `ChatMessage.content` is a `string` per ADR 0020 D6 — so a declared image modality records a capability A008 cannot yet use. |
