...
A008-0114 is chartered as Stage 3.5 between completed V2/auth Stage 3 and unstarted Stage 4. ADR 0043 accepts an immutable evaluation boundary: an ACME integration may replace provider execution only; A008 retains all cognition, context/retrieval, tool authority, orchestration and memory semantics. The task is blocked on ACME-0176 exposing a versioned model-only execution runtime. No A008 ACME adapter exists yet, direct provider transports remain current behavior, and `SYSTEMDOC.md` therefore remains unchanged until implementation evidence exists. Stage 3.5 will compare direct and ACME execution for semantic parity and failure evidence and record go/no-go before Stage 4 begins.
