...
Reality as of 2026-09-17. This document records observed state; intended design
...
| ACME execution adapter | A008-0118. Opt-in `AcmeChatTransport` maps prepared A008 `ChatRequest` values to `acme-model-runtime/2`, including `topP` / `reasoningBudget` / `enableThinking` / `reasoningEffort` / `seed` when the model honors them. `executionProvider` is the only source of ACME `providerHint` (`openai`, `nvidia`, or `kie:<model>`). Compatibility is checked before execute. Typed ACME diagnostics surface as `AcmeChatError` with `modelExecutionId`/delivery evidence. Stage 3.5 **GO**: live chat matrix and owner client loop passed; images stay on direct transports. |
