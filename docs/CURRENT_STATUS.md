...
A008-0114 completed the first Stage 3.5 evaluation against frozen `acme-model-runtime/1` and recorded **NO-GO** because that wire could not carry A008 thinking/reasoning/topP/seed controls. A008-0118 upgrades the same adapter to `acme-model-runtime/2` and closes that mapping gap offline. Direct NVIDIA/kie/OpenAI transports remain the default until live matrix evidence and the owner client loop are recorded. Stage 4 remains unstarted.
...
| Shared provider composition | `createNvidiaChatTransport` owns NVIDIA credential validation, model defaults, optional trusted endpoint override, and construction of the existing adapter. Live CLI/ACP without an injected transport use `createConfiguredChatTransport`, which defaults to `createDispatchingChatTransport` (NVIDIA, kie.ai, or OpenAI from the selected model/user catalog). `A008_CHAT_TRANSPORT=acme` selects `AcmeChatTransport` against `acme-model-runtime/2` instead; there is no post-dispatch fallback to a direct provider. `createLocalMemoryRuntime` injects that one transport into CLI and ACP memory turns. Direct composition still needs one of `NVIDIA_API_KEY`, `KIE_API_KEY`, or `OPENAI_API_KEY`. ACME composition uses `A008_ACME_MODEL_RUNTIME_URL` and does not send provider keys in model content. Core remains environment-neutral. |
| ACME execution adapter | A008-0118. Opt-in `AcmeChatTransport` maps prepared A008 `ChatRequest` values to `acme-model-runtime/2`, including `topP` / `reasoningBudget` / `enableThinking` / `reasoningEffort` / `seed` when the model honors them. `executionProvider` is the only source of ACME `providerHint` (`openai`, `nvidia`, or `kie:<model>`). Compatibility is checked before execute. Typed ACME diagnostics surface as `AcmeChatError` with `modelExecutionId`/delivery evidence. Default-route adoption remains pending live matrix and owner client-loop evidence. |
