...
A008-0114 completes Stage 3.5. `AcmeChatTransport` speaks frozen `acme-model-runtime/1` (`GET /v1/model/compatibility`, `POST /v1/model/execute`) beneath the existing `ChatTransport` contract. It is selected only by explicit `A008_CHAT_TRANSPORT=acme`; direct NVIDIA/kie/OpenAI transports remain the default and the authoritative execution path. Deterministic parity and failure-evidence tests pass. Adoption decision: **NO-GO** for making ACME the normal route. Frozen ACME `ModelRequest` cannot carry A008 generation controls beyond temperature, max output tokens and stop, so default NVIDIA thinking/reasoning profiles would silently lose provider-facing controls. ACME-0176's protocol is frozen and consumed here; ACME's own task is not archived Complete. Stage 4 remains unstarted and proceeds without ACME as the execution substrate.
...
| Shared provider composition | `createNvidiaChatTransport` owns NVIDIA credential validation, model defaults, optional trusted endpoint override, and construction of the existing adapter. Live CLI/ACP without an injected transport use `createConfiguredChatTransport`, which defaults to `createDispatchingChatTransport` (NVIDIA, kie.ai, or OpenAI from the selected model/user catalog). `A008_CHAT_TRANSPORT=acme` selects `AcmeChatTransport` against `acme-model-runtime/1` instead; there is no post-dispatch fallback to a direct provider. `createLocalMemoryRuntime` injects that one transport into CLI and ACP memory turns. Direct composition still needs one of `NVIDIA_API_KEY`, `KIE_API_KEY`, or `OPENAI_API_KEY`. ACME composition uses `A008_ACME_MODEL_RUNTIME_URL` and does not send provider keys in model content. Core remains environment-neutral. |
| ACME execution adapter | A008-0114. Opt-in `AcmeChatTransport` maps prepared A008 `ChatRequest` values to `acme-model-runtime/1`, streams reasoning/content, returns structurally complete tool calls, and sends tool-result continuation as a later bounded execute. Compatibility is checked before execute. Typed ACME diagnostics surface as `AcmeChatError` with `modelExecutionId`/delivery evidence. Default-route adoption is NO-GO; Stage 4 does not use ACME. |
