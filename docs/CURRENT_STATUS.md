...
A008-0131 (implementation branch; owner live A/B pending) moves embedded GPT-5.6 Luna back to ACME's native OpenAI Responses route on registry `acme-engine@0.1.5`. A008-owned OpenAI profiles now compose under `config.openAi`; NVIDIA remains on its existing Chat Completions route and KIE remains under compatible routes. The provider-neutral ACME request mapper no longer applies the Luna+function-tools Chat Completions workaround, so Responses receives the selected reasoning effort; the direct `OpenAiChatTransport` remains Chat Completions and still forces effective `reasoning_effort=none` for Luna tool requests. Focused offline proof passed 51/51 and drove `retrieval_scope -> chat -> knowledge_analysis -> relation_classification -> commit` entirely through `https://api.openai.com/v1/responses`; semantic calls retained `reasoning.effort=none`, normal chat retained `medium`, and mixed text/image plus function tools mapped through the native Responses adapter.
