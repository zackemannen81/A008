...
- Image uploads are stored but not extracted. The `ImageDescriber` port and
  ...
  authority.
- Retrieval matches on entities only. Tags and domains are extracted and then
  never stored, so knowledge related to the current topic is not pulled in
  unless it shares an entity with the message. ADR 0023 D5 has the detail; it is
  the next task.
- `Entity.labels` is polluted by `tokenize(proposition)` in `live-commit.ts`,
  which keeps every word of four characters or more. The word `heter` is
  therefore an alias of any fact whose proposition contains it, and `"Vad heter
  du?"` matches them all. Lexical search terms do not belong in semantic
  identity; this is the same task.
